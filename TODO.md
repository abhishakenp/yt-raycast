...
|   6 | Billing, Credits, And Checkout                    | P0       | A Convex billing API surface now exists for signed-in subscription status, credit balance, and billing overview, and export entitlement reads existing `subscriptions` / `customerCredits` records instead of hardcoding `requiresPayment: false`. The app now exposes `/api/subscription-status`, `/api/credits`, `/api/billing-overview`, `/api/checkout/start`, `/api/stripe/webhook`, and `/api/razorpay/webhook`. Checkout start uses direct Stripe/Razorpay REST calls with authenticated Convex user resolution, and webhooks verify provider signatures before calling a secret-gated Convex mutation that idempotently upserts subscriptions or credits. | Partial / credit ledger added.                  | Added `consumeCreditForExport` internal mutation for credit consumption during exports with insufficient credit error handling. Added creditLedger table to Convex schema with userId, sessionId, amount, balanceAfter, reason, createdAt fields. Enhanced `consumeCreditForExport` to record transactions in creditLedger. Enhanced `getCreditLedger` query to return full transaction history with pagination. Remaining gaps: real Stripe/Razorpay sandbox credentials run, checkout UI wiring, early adopter slot accounting, invoice/subscription lifecycle event breadth, production webhook secret configuration, end-to-end paid export verification in browser. |
