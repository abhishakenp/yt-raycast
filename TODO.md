...
|   9 | Medusa Ecommerce Provisioning And Storefront Sync | P1       | Current commerce support stores backend/admin/storefront URLs and config JSON. It does not provision tenants, extract/sync products, expose storefront config, or proxy cart/checkout behavior.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Partial / storefront config and cart routes added. | Added provisionMedusaTenant internal mutation that creates or updates commerce config records with backend/admin/storefront URLs. Added syncMedusaProducts internal mutation that stores product data with count tracking. Added storefront config endpoint at /api/medusa-store/config. Added cart endpoints at /api/medusa-store/cart (POST), /api/medusa-store/cart/$id (GET), /api/medusa-store/cart/line-items (POST). Enhanced commerceConfigs table usage for tenant management. Remaining gaps: checkout proxy, admin embed config, acceptance tests, and browser verification.                                                                                                                                                     |
