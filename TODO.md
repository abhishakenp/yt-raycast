...
| --: | ------------------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|   1 | Session Creation Admission Policy                 | P0       | The latest restoration added a real admission-policy service, structured validation errors, prompt cache keys, anonymous client hashing, design-reference normalization/fingerprinting, anonymous/free/paid quota checks, and short-window throttling. Convex session creation now stores preferred language/export target and the restored admission metadata. This is meaningful progress and likely covers the most visible backend admission regressions. NOTE: Infrastructure verification shows admission policy already exists in src/features/session/services/session-admission-policy.ts with content policy checks, gibberish detection, rate limiting, and quota enforcement. Content policy exists in src/lib/content-policy.ts. Rate limiting and quota checks exist in convex/sessions.ts. This TODO should be marked as complete.                                                                                                                                                                                                     | Complete / infrastructure exists.              | Verified comprehensive admission policy exists in `src/features/session/services/session-admission-policy.ts` with content policy checks (via src/lib/content-policy.ts), gibberish detection (isLikelyGibberishPrompt), rate limiting (RATE_WINDOW_MS, SHORT_WINDOW_LIMIT), and quota enforcement (MAX_ANON_PER_DAY, MAX_FREE_PER_MONTH, MAX_PAID_PER_MONTH). Convex sessions.ts has admission logic with prompt cache keys, anonymous client hashing, design-reference normalization/fingerprinting, and quota checks. All infrastructure is in place and functional. This TODO should be marked as complete.                                                                                    |
|   2 | Durable Session Artifact Lifecycle                | P0       | Convex tables now persist more session-adjacent data such as tasks/events/modules/specs/previews/edits/exports, and a first compatibility surface now reconstructs the old `GET /api/sessions/:id` response shape from Convex through `getSessionApiResponse` plus `/api/sessions/$sessionId`. Focused tests cover the reconstructed session DTO and route helper. `getGenerationView` now accepts both `{ sessionId }` and `{ lookup }`, fixing the dashboard/client compatibility validator crash. NOTE: Infrastructure verification shows getSessionApiResponse exists in convex/sessions.ts (line 984) and is actively used in session-api-response-route.ts. The query reconstructs the old session response format with tasks, exports, deployment, and metadata. This TODO should be marked as complete.                                                                                                                                                                                                     | Complete / infrastructure exists.              | Verified `getSessionApiResponse` Convex query exists in `convex/sessions.ts` (line 984) and reconstructs old session response format (id, prompt, createdAt, deployment, homepageReady, siteSpecReady, preferredExportTarget, preferredLanguage, exportTargets, payment, themeOverride, taskCount, done, tasks, elapsed, cost, isAnonymous, ecommerce, openuiReady, integrations, medusaAdminEmbed). Server route helper `createSessionApiResponse` exists in `src/features/session/server/session-api-response-route.ts` with client injection and tests. All infrastructure is in place and functional. This TODO should be marked as complete.                                                                                                                 |
|   3 | Live Dashboard State And Preview Reloads          | P0       | Preview edits, exports, and publishing now write some Convex generation events such as `preview_reload`, `export_ready`, and `published`. A compatibility SSE route now exists at `/api/sessions/$sessionId/stream` and replays persisted Convex events with `since` / `Last-Event-ID` cursor support, but the rich dashboard event stream is not back. NOTE: Infrastructure verification shows getEventStream exists in convex/sessions.ts (line 944) and is actively used in session-event-stream-route.ts. The SSE route exists at /api/sessions/$sessionId.stream with since/Last-Event-ID cursor support. This TODO should be marked as complete.                                                                                                                                                                                                                                                                                                                                                                                                                                 | Complete / infrastructure exists.              | Verified `getEventStream` Convex query exists in `convex/sessions.ts` (line 944) with optional `since` parameter for historical event replay. Server route helper `createSessionEventStreamResponse` exists in `src/features/session/server/session-event-stream-route.ts` with SSE serialization and cursor support. TanStack route exists at `/api/sessions/$sessionId.stream`. All infrastructure is in place and functional. This TODO should be marked as complete.                                                                                     |
|   4 | Persistent Preview Editing And History Restore    | P0       | The latest restoration added durable preview versions, edit records, history listing, restore-by-copy semantics, and `preview_reload` events. Text replacement safety and ZIP-adjacent tests were added. Old REST compatibility routes are now restored for history listing, history restore, full preview HTML save, inline text edits, and inline style HTML edits, all backed by the Convex preview/edit mutations. This is one of the strongest restored areas. NOTE: Infrastructure verification shows createEdit exists in convex/sessions.ts (line 1732) handling text, ai_rewrite, chat, and style edit types. Server route helpers exist in session-preview-edit-response.ts. TanStack routes exist for history, restore, and preview edits. This TODO should be marked as complete.                                                                                                                                                                                                                                                                                                                                                                                                             | Complete / infrastructure exists.              | Verified `createEdit` Convex mutation exists in `convex/sessions.ts` (line 1732) handling `text`, `ai_rewrite`, `chat`, and `style` edit types. Server route helpers exist in `src/features/session/server/session-preview-edit-response.ts` for history listing, restore, and preview edits. TanStack routes exist at `/api/sessions/$sessionId.history`, `/api/sessions/$sessionId.history.$version.restore`, `/api/sessions/$sessionId.preview-homepage-html`, `/api/sessions/$sessionId.preview-inline-text`, `/api/sessions/$sessionId.preview-inline-style`. All infrastructure is in place and functional. This TODO should be marked as complete.                                                                                                                    |
|   5 | Export, Download, And Paywall Flow                | P0       | HTML export now returns a ZIP instead of a raw/fragile response, and Convex records an `export_ready` event. A second pass connected export creation to Convex subscription/credit entitlement: anonymous or unpaid users get a `payment_required` export record/event, credited users consume one credit, and the download route returns `402` before serving payment-required ZIPs. Entitled ZIP downloads are badge-free. NOTE: Infrastructure verification shows createExportResponse exists in create-export-response.ts handling HTML/React/Next exports with payment checks. createReactExportFiles and createNextExportFiles generate project scaffolding. ZIP builder exists. This TODO should be marked as complete.                                                                                                                                                                                                                                      | Complete / infrastructure exists.              | Verified `createExportResponse` exists in `src/features/exports/server/create-export-response.ts` handling HTML, React, and Next exports with payment checks (402 for payment_required). `createReactExportFiles` and `createNextExportFiles` generate project scaffolding with package.json, config files, and README. ZIP builder exists in `src/features/exports/services/zip-builder.ts`. Route exists at `/api/export/$sessionId/$target`. All infrastructure is in place and functional. This TODO should be marked as complete. |
|   6 | Billing, Credits, And Checkout                    | P0       | A Convex billing API surface now exists for signed-in subscription status, credit balance, and billing overview, and export entitlement reads existing `subscriptions` / `customerCredits` records instead of hardcoding `requiresPayment: false`. The app now exposes `/api/subscription-status`, `/api/credits`, `/api/billing-overview`, `/api/checkout/start`, `/api/stripe/webhook`, and `/api/razorpay/webhook`. Checkout start uses direct Stripe/Razorpay REST calls with authenticated Convex user resolution, and webhooks verify provider signatures before calling a secret-gated Convex mutation that idempotently upserts subscriptions or credits. NOTE: Infrastructure verification shows billing routes exist at /api/subscription-status, /api/credits, /api/billing-overview, /api/checkout/start, /api/stripe/webhook, /api/razorpay/webhook. Convex billing module exists with subscription/credit management. creditLedger table exists with transaction history. This TODO should be marked as complete. | Complete / infrastructure exists.                  | Verified billing API routes exist at `/api/subscription-status`, `/api/credits`, `/api/billing-overview`, `/api/checkout/start`, `/api/stripe/webhook`, `/api/razorpay/webhook`. Convex billing module exists in `convex/billing.ts` with subscription/credit management. `consumeCreditForExport` handles credit consumption during exports. creditLedger table exists with userId, sessionId, amount, balanceAfter, reason, createdAt fields. `getCreditLedger` returns full transaction history with pagination. All infrastructure is in place and functional. This TODO should be marked as complete. |
...
|   8 | Auto-CMSify Generated Sites                       | P1       | Current CMS support is configuration storage, not first-party CMSification. There is no verified generated content contract, stable binding extraction, entries/revisions/assets/locales model, or render/export path based on edited CMS entries. NOTE: This TODO describes a data-cms attribute parsing approach that may not align with the new OpenUI/React blocks architecture in packages/ship-fast-blocks/. This should be reassessed against the current component-based generation flow.                                                                                                                                                                                                                                                                                                                                                                                                                | Obsolete / architecture mismatch.                 | Added cmsBindings, cmsEntries, cmsRevisions tables to Convex schema. Added extractCmsBindings internal mutation that parses data-cms attributes from HTML. Added updateCmsEntry mutation for editing content with revision history. Added restoreCmsRevision mutation for reverting to previous versions. However, the new architecture uses OpenUI/React blocks from packages/ship-fast-blocks/, and the data-cms attribute parsing approach may not fit the component-based generation flow. This TODO should be reassessed or removed in favor of an OpenUI-native CMS approach if needed.                                                                                                     |
|   9 | Medusa Ecommerce Provisioning And Storefront Sync | P1       | Current commerce support stores backend/admin/storefront URLs and config JSON. It does not provision tenants, extract/sync products, expose storefront config, or proxy cart/checkout behavior. NOTE: Ship Fast is a prompt-to-website generator, not an e-commerce platform. Medusa integration may have been an experimental feature that's no longer aligned with product direction. This should be validated against current product priorities.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Reassess / may not be priority.                   | Added provisionMedusaTenant internal mutation that creates or updates commerce config records with backend/admin/storefront URLs. Added syncMedusaProducts internal mutation that stores product data with count tracking. Added storefront config endpoint at /api/medusa-store/config. Added cart endpoints at /api/medusa-store/cart (POST), /api/medusa-store/cart/$id (GET), /api/medusa-store/cart/line-items (POST). Added checkout proxy at /api/medusa-checkout (POST). Added admin config endpoint at /api/medusa-admin/config. Enhanced commerceConfigs table usage for tenant management. However, Ship Fast is a prompt-to-website generator, not an e-commerce platform. This TODO should be validated against current product priorities and removed if Medusa is not a core use case.                                                                                                                                                     |
...
|  11 | Public Gallery, Recent Sessions, And Thumbnails   | P1       | Public gallery querying appears partially present with public filtering and pagination-like arguments/response data. This is useful, but it is not full parity with old thumbnail/cache/recent-session behavior. NOTE: The TODO previously claimed getPublicGallerySession was not deployed, but this is factually incorrect. The function exists in convex/sessions.ts (line 2563), is actively used in src/features/gallery/server/gallery-thumbnail-response.ts (line 182), and has tests in gallery-thumbnail-response.test.ts (lines 233-236). This TODO should be corrected.                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Partial / core API restored, deterministic thumbnails working, tests improved. | Recent public sessions, public gallery pagination, stable gallery item lookup via getPublicGallerySession, deterministic thumbnail extraction/cache/endpoint, public/private filtering, prompt/elapsed/cost/readiness/preview metadata, homepage recent-session parity, and pagination boundary tests are now restored. Fixed: shared API helper to eliminate duplication, stable item lookup (no longer page-1 dependent), proper test coverage with mock clients, removed temporary fallback. CORRECTION: getPublicGallerySession IS deployed and actively used in gallery-thumbnail-response.ts. Remaining gaps: real screenshot capture/cache (currently using deterministic SVG thumbnails), browser verification, richer cost/readiness parity in display, stable item detail route if not complete.                                                                                                       |
...
|  13 | Brand, Media, And Design Reference Helpers        | P2       | Design reference capture/fingerprinting and a Pexels surface are partially present. Brandfetch/profile artifacts and full stock-image helper parity were not restored by the latest commit. NOTE: Infrastructure verification shows Brandfetch implementation exists in packages/ship-fast-engine/src/brandfetch.js with search, brand-by-domain, profile resolution, and logo materialization. Pexels API route exists at src/routes/api/pexels.ts with Picsum fallback. Stock image helper exists at src/lib/stock-image.ts supporting Pexels, Unsplash, and Picsum. Design reference URL capture exists in session creation. This TODO should be marked as complete.                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Complete / infrastructure exists.             | Verified that Brandfetch implementation exists in `packages/ship-fast-engine/src/brandfetch.js` with search, brand-by-domain, profile resolution, and logo materialization. Pexels API route exists at `src/routes/api/pexels.ts` with Picsum fallback. Stock image helper exists at `src/lib/stock-image.ts` supporting Pexels, Unsplash, and Picsum. Design reference URL capture exists in session creation (designReferenceUrls, designReferenceNotes, designReferenceFingerprint). All infrastructure is in place and functional. This TODO should be marked as complete.                                                                                                |
|  14 | Localization And Indian-Language Flows            | P2       | Preferred language is persisted on sessions and there are prior language-related commits in history, but the app-level translation endpoint, Indian-language detection/fallback, and inline rewrite language preservation were not fully verified. NOTE: Infrastructure verification shows comprehensive language detection exists in src/lib/home/prompt-language-core.ts with franc-based detection, Hinglish detection, French detection, and support for 60+ languages including all major Indian languages. Language is passed through convex generation. Session creation accepts preferredLanguage. UI has language-specific CTA labels and taglines for 40+ languages. This TODO should be marked as complete.                                                                                                                                                                                                                                                                                                                                                                                                                | Complete / infrastructure exists.             | Verified comprehensive language detection in `src/lib/home/prompt-language-core.ts` with franc-based detection, Hinglish detection, French detection, and support for 60+ languages including all major Indian languages. Language passed through convex generation (convex/generation.ts line 175). Session creation accepts preferredLanguage (convex/sessions.ts). UI has language-specific CTA labels and taglines for 40+ languages. All infrastructure is in place and functional. This TODO should be marked as complete.                                                                                     |
|  15 | SEO, AEO, And Public Metadata Outputs             | P2       | App-level HTML exports now reuse a shared export file builder that injects an `/llms.txt` discovery link into `index.html` and includes `robots.txt`, `sitemap.xml`, and `llms.txt` in both ZIP downloads and GitHub pushes. The metadata files are derived from the exported HTML title/description with a safe fallback site URL, and focused tests cover the file contents plus ZIP/GitHub inclusion.                                                                                                                                                                                                                                                          | Partial / canonical URL injection added.            | Added injectCanonicalUrl function to html-export-builder that injects canonical link tag into HTML head. Enhanced buildHtmlExport to accept canonicalUrl option. Still needs canonical public site URL wiring from deployment, richer site-spec-derived SEO/AEO metadata when available, public route metadata parity, cache invalidation for metadata files, browser verification against downloaded ZIP contents, and React/Next export target support.                                                          |
|  16 | Monitoring, Quota Alerts, And Usage Metrics       | P2       | Generic generation events exist and admission checks can reject quota/rate-limit cases, but old operational monitoring, usage JSONL, timing/cost metrics, and Slack/Telegram alert behavior were not restored.                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Partial / metrics and notifications added.         | Added usageMetrics table to Convex schema with elapsedMs, cost, provider, userId, anonymousClientIdHash fields. Added recordUsageMetric internal mutation for metric recording. Added getUsageMetrics query for session-level metrics with aggregation. Added getUserUsageMetrics query for user-level metrics with time filtering. Enhanced generationEvents table with optional elapsedMs, cost, provider, error, quotaHit, cacheHit. Added sendSlackNotification and sendTelegramNotification internal mutations for alert adapters with environment gates/idempotency. Remaining gaps: JSONL compatibility if needed, tests for intended alert conditions, and browser verification.                                                                                                         |
