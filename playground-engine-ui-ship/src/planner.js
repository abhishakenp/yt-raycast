...
export function fallbackGenome(brief, route, variety, grammar) {
  ...
  return {
    ...
    mediaStrategy: blog
      ? {
          featuredKind: grammar?.mediaKinds?.[0] || 'article-cover',
...
export function buildPlannerPrompt(brief, route, variety, grammar) {
  const quality = process.env.SHIP_FAST !== '1' && process.env.KIMI_FAST !== '1'
...
export async function planPageGenome({ brief, route, variety, grammar, llm }) {
  ...
  const result = await llm({
    ...
    prompt,
    temperature: quality ? 0.88 : 0.55,
    maxTokens: quality ? 1600 : 1000,
    reasoningEffort: 'low',
