...
export function validateFullWidthSections(html, { minSections = 6, allowImg = false } = {}) {
  const issues = []
  ...
  if (!allowImg && /<img\b/i.test(html)) issues.push('img tag')
...
export function runDeterministicAudits(html, { plan, route, seed, brief } = {}) {
  ...
  const structure = validateFullWidthSections(html, {
    ...
    allowImg: publication.skipped === false,
  })
