...
import { componentNames } from "@ship-fast/blocks"
import { validateOpenUISource } from "../pipeline/openui-validate.js"
import { generateFallbackSection, contentCoverage, expectedContentStrings, duplicationRatio, hallucinationRatio } from "./fallback.ts"
...
const KNOWN_PRIMITIVES = new Set(componentNames)
...
function validateSectionProgram(program: string, sectionVar: string): string | null {
  ...
  if (/^\s*root\s*=/m.test(codeText)) return "section program must not define root"

  const topLevelDefs = [...text.matchAll(/^\s*([A-Za-z][A-Za-z0-9_]*)\s*=/gm)].map((m) => m[1])
  ...
  const calls = [...codeText.matchAll(/\b([A-Za-z][A-Za-z0-9_]*)\s*\(/g)].map((m) => m[1])
...
export async function convertSection(
  ...
  try {
    ...
    if (reason) {
      ...
      return { ...generateFallbackSection(section.kind, pageUrl, section.startIndex, tokens, section.html), hash, sourceHtml }
    }
    ...
    if (expectedCount >= 1) {
      ...
      if (thin || duplicated || fabricated) {
        ...
        const fixesDup = duplicated && recDup < dup && recCoverage >= coverage * 0.9
        ...
        const fixesFab = fabricated && recHalluc < halluc && recCoverage >= coverage * 0.9
