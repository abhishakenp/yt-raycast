...
export function buildCloneBrief(capture: CapturedPage): string {
  ...
  if (document) {
    ...
    for (const h of headingEls) {
      ...
      sectionLines.push({
        heading: headingText,
        lead: truncate(lead, MAX_LEAD_CHARS),
      })
    }
  }
  ...
  lines.push(
    'Reproduce this structure and content faithfully using native sections.',
  )

  const brief = lines.join('\n')
  return brief.length > MAX_BRIEF_CHARS
    ? truncate(brief, MAX_BRIEF_CHARS)
    : brief
}
