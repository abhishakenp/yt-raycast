...
export function extractPlan(skeleton: string): PlanItem[] {
  ...
  if (navGroup && pageGroup) {
    const labels = [...navGroup.matchAll(STRING_RE)].map((m) => m[0].slice(1, -1))
    const ids = pageGroup
      .split(",")
      .map((s) => s.trim())
      .filter((s) => /^[A-Za-z_]\w*$/.test(s))
    if (ids.length > 0) return ids.map((id, i) => ({ id, label: labels[i] ?? humanize(id) }))
  }
...
export function extractChildIds(skeleton: string): string[] {
  ...
  for (const rawLine of skeleton.split("\n")) {
    ...
    const rhs = (def ? line.slice(line.indexOf("=") + 1) : line).replace(STRING_RE, " ")
