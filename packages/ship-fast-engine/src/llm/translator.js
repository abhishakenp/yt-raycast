...
const parseTranslationResponse = (content, records) => {
  const jsonMatch = String(content || '').match(/\{[\s\S]*\}/)
  if (!jsonMatch) return {}

  const parsed = JSON.parse(jsonMatch[0])
  const translatedById = parsed.translations || parsed
  const out = {}
...
const countScriptLetters = (text, script) => {
...
const isAlreadyLocalized = (texts, language) => {
...
async function polishTranslations(texts, draftTranslations, language) {
  const records = buildTextRecords(texts)
  const items = records
    .map((record) => ({
      id: record.id,
...
const buildTranslationItems = (texts, translations) =>
...
async function scoreTranslations(texts, translations, language) {
 ...
 */
export async function translateHtml(html, languageMode) {
