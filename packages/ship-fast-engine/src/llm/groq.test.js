...
const completion = (content, usage = {}) => ({
  json: async () => ({
    usage: {
      prompt_tokens: 100,
...
const errorCompletion = (message) => ({
  json: async () => ({
    error: { message },
  }),
})

const { groq, groqStream, groqParallel } = await import('./groq.js?groq-test')
...
describe('groq', () => {
  ...
  it('routes cloud-suffixed models through the Ollama-compatible endpoint', async () => {
    fetch.mockResolvedValueOnce(completion('Cloud answer'))
  ...
  it('returns provider errors without pretending there is content', async () => {
    fetch.mockResolvedValueOnce(errorCompletion('model overloaded'))
