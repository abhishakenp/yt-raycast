...
let _anonKey = ''
let _searchableAppsCache = new Map()
...
export function writeAuthToDisk(auth, file = authFilePath()) {
  try {
 ...
 */
export function importSessionFromBrowserCookie(cookieHeader) {
...
function sessionToCookieHeader(auth) {
...
function mobbinApiHeaders(cookie) {
  return {
    Cookie: cookie,
...
async function mobbinApiFetch(path, { method = 'GET', body } = {}) {
...
function normAppName(name) {
  return String(name || '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '')
}

function mapPreviewScreen(appName, screen) {
  return {
    app: appName,
    patterns: [],
    elements: [],
...
function flattenPopularScreens(apps, limit) {
  const out = []
  const seen = new Set()
...
export async function autocompleteMobbinApp(query, { platform = 'web' } = {}) {
