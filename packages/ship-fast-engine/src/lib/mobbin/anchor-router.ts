...
function normalizeBriefText(brief = '', projectContext: ProjectContext = {}) {
  return [
    brief,
...
export function anchorMatchesBrief(
  brief: string,
...
function categoryOf(app: string): string {
 ...
 */
export async function inferMobbinAnchor({
  brief,
  projectContext = {},
}: { brief?: string; projectContext?: ProjectContext } = {}) {
...
Return JSON only.`
  ...
  const accents = Array.isArray(dna.accents)
    ? dna.accents.filter((h: string) => /^#[0-9a-f]{6}$/i.test(h))
    : []
