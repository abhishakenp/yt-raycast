...
function paletteRoleHint(palette: string[] = []) {
  const norm = palette
    .map((h) => h.toLowerCase())
...
type DnaDescriptor = MobbinDna
type Anchor = MobbinAnchor

function dnaImperatives(dna: DnaDescriptor | null, app: string): string[] {
  if (!dna) return []
  const out = []
...
function dnaCompositionBlock(dna: DnaDescriptor | null, _app: string): string {
 ...
 */
export function dnaSectionsBlock(
  dna: DnaDescriptor | null,
  app: string,
): string {
...
export function mobbinSessionBlock(anchor: Anchor | null): string {
