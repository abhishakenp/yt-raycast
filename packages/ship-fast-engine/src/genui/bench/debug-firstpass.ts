...
const candidates = shortlist.map((fam) => ({
  ...
  lines: fam.sections.map(
    (sec) => `  ${sec.toLowerCase()}: ${getComponentSignature(`${fam.name}${sec}`) ?? ''}`,
  ),
}))
...
const raw = await generateText(model, sys, user, new AbortController().signal, 1)
...
try {
  ...
  console.log('parsed family:', obj.family, '| section keys:', Object.keys(obj.sections || {}).join(','))
} catch (e) {
