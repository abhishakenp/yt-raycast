...
export function loadDesignRef(name) {
  if (!name) return null
...
export const getAuroraDesignRef = () => loadDesignRef('aurora')

export function stashDesignRefName(workspace, name) {
  if (!workspace || !name) return
  writeFile(workspace, WORKSPACE_FILE, name)
}

export function readDesignRefFromWorkspace(workspace) {
...
export function designRefSystemAppendix(designRef) {
