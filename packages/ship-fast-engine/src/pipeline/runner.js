...
// import { generateAndWriteOpenUIHome } from './phase-openui-home.js'
import {
...
export async function runEdit({ prompt, workspace, sessionCtx, integrations } = {}) {
    ...
    if (integrations?.afterSiteSpecSaved) {
      try {
        await integrations.afterSiteSpecSaved({
          workspace,
          siteSpec: enrichedSiteSpec,
          log: _log,
          status: _status,
        })
      } catch (err) {
        _log(`  integrations: afterSiteSpecSaved failed – ${err?.message || err}`)
      }
    }
...
export async function runAll({ prompt, workspace, sessionCtx, preferredLanguage, integrations } = {}) {
  const _log = log(sessionCtx)
  const _status = status(sessionCtx)
  const t0 = Date.now()
  const normalizedPrompt = requirePromptText(prompt)
  ...
  if (indiaMode.code !== 'en') {
    _log(`  Language detected: ${indiaMode.name} (${indiaMode.code})`)
  }
  ...
  if (siteSpec) {
    ...
    if (integrations?.afterSiteSpecSaved) {
      try {
        ...
        console.warn(`integrations: afterSiteSpecSaved failed – ${err?.message || err}`)
      }
    }

    // Generate OpenUI home page
    try {
      _status('Generating OpenUI components...', 'openui')
      // OpenUI generation temporarily disabled during enhanced implementation
      _log('  OpenUI generation skipped - using enhanced pipeline')
    } catch (err) {
      _log(`  OpenUI generation failed – ${err?.message || err}`)
    }
  }
