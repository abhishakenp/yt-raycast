...
export const promptExpectsNovaDenseMarketing = (prompt) => {
  ...
  if (/\b(government\s+portal|civic\s+portal|ministry\b.*\bportal)\b/.test(p)) return false
...
export const explainNovaMarketingBarFailures = (html) => {
  ...
  const hasVisualHook =
    ...
    ((raw.match(/bg-gradient-to-/gi) || []).length >= 2 && /bg-gradient-to-/i.test(raw)) ||
...
export const htmlFailsNovaMarketingBar = (html) => explainNovaMarketingBarFailures(html).length > 0
