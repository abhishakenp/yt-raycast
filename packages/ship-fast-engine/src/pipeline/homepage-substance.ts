...
function looksLikeThreeJsGame(html: string): boolean {
  return (
...
function looksLikeDeclaredAppUi(html: string): boolean {
  const h = String(html || '')
  return (
...
function hasMarketingStructure(bodyHtml: string): boolean {
...
function looksLikeSubstantialEcommerceHomepage(html: string): boolean {
...
function looksLikeHybridLlmHomepage(html: string): boolean {
  const h = String(html || '')
...
export function shouldReplaceLlmHomepageWithRenderer(
  html: string,
  siteSpec: SiteSpec,
): boolean {
...
export function htmlDocumentPassesPreviewQuality(
  html: string,
  siteSpec: SiteSpec,
): boolean {
  return Boolean(
