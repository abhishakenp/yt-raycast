...
const collectNovaVisualCraftIssues = (html, siteType) => {
  ...
  if (!/(blur-3xl|blur-\[\d)/i.test(s)) {
    issues.push('Nova-tier: stack multiple blurred gradient layers (blur-3xl or blur-[…px]) behind hero content')
  }
  ...
  if (reveals < 4) {
    issues.push(`Nova-tier: need >=4 scroll-reveal blocks (data-reveal + IntersectionObserver); have ${reveals}`)
  }
  const magnets = (markup.match(/\bdata-magnet\b/gi) || []).length
  if (magnets < 2) {
    issues.push(`Nova-tier: wire data-magnet on at least two primary CTAs with pointer parallax (have ${magnets})`)
  }
  const hasKeyframeBlock = /keyframes\s*:\s*\{/.test(s)
  const hasLiquidMotion =
    /\banimation:\s*liquid\b|\[animation:\s*liquid\b|animate-liquid\b|keyframes[\s\S]{0,6000}\bliquid\b/i.test(s) ||
  ...
  const diagonal =
    /skew-(?:x-|y-)?(?:1|2|3|6|12)|-skew|rotate-(?:1|2|3|6)|clip-path|polygon\(|skewY|skewX/i.test(markup) ||
  ...
  if (
    /<p[^>]*(?:text-lg|text-base|max-w-xl|max-w-lg|max-w-md|leading-relaxed)[^>]*\btext-slate-500\b/i.test(markup)
  ) {
    issues.push(
...
export const collectHomepageQualityIssues = (html, { siteType = '', prompt = '' } = {}) => {
  const s = String(html || '')
  const issues = []
  const st = String(siteType || '').toLowerCase()
  if (st === 'game') return issues

  if (!/name=["']viewport["']/i.test(s)) issues.push('missing viewport meta in head')
  ...
  const inlineScripts = s.match(/<script(?![^>]*\bsrc\s*=)[^>]*>[\s\S]*?<\/script>/gi) || []
  const inlineLen = inlineScripts.join('').length
  if (inlineLen < 280) issues.push('homepage needs a substantive inline <script> (vanilla JS for nav/pricing/FAQ/carousel)')
  ...
  if (btnSiteTypes.includes(st) && btn < minBtn) issues.push(`too few <button> elements (${btn}); add real CTAs`)

  const links = (s.match(/<a\s[^>]*href=/gi) || []).length
  const minLinks = st === 'dashboard' ? 2 : 8
  if (links < minLinks) issues.push(`too few navigational links (${links}); add dense header/footer routes`)

  const low = stripTags(s)
  if (/\blorem ipsum\b/.test(low)) issues.push('remove lorem ipsum placeholder copy')
  if (/\bplaceholder text\b/.test(low)) issues.push('remove generic placeholder copy')
  ...
  if (/\b(ecommerce|store|shop|cart|retail)\b/.test(p) || st === 'ecommerce') {
    if (!/data-open-drawer|data-cart|cart-toggle|data-add\b/i.test(s))
      issues.push('ecommerce: add cart or add-to-cart wiring (data-open-drawer, data-cart-*, or data-add)')
  }
