...
export function stripStorefrontCartUi(html: string): string {
...
function readVariantMapFromWorkspace(
  workspace: string,
...
export function injectMedusaVariantDataAttributes(
  html: string,
...
function buildBlock(medusaMapJson: string): string {
...
export function injectStorefrontCartUi(
  html: string,
  options: {
    force?: boolean
    variantMap?: Record<string, unknown> | null
    workspace?: string
  } = {},
): string {
  if (!html || typeof html !== 'string') return html
  ...
  let map: Record<string, unknown> | null = options.variantMap || null
