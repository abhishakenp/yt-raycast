...
function normalizeSection(section, idx = 0) {
  const type = SUPPORTED_SECTION_TYPES.includes(section?.type)
    ? section.type
    : 'features'
  return {
    ...
    styling:
      typeof section?.styling === 'object' && section?.styling
        ? section.styling
        : {},
    visibility:
      typeof section?.visibility === 'object' && section?.visibility
        ? section.visibility
        : {},
    contentBlocks: ensureArray(section?.contentBlocks).map(
      ...
      section?.form && typeof section.form === 'object'
        ? {
            successMessage: ensureString(
              section.form.successMessage,
              'Submitted successfully.',
            ),
            errorMessage: ensureString(
              section.form.errorMessage,
              'Unable to submit.',
            ),
            action:
...
function normalizeEcommerce(raw, fallback) {
  const fromRaw =
    raw?.ecommerce && typeof raw.ecommerce === 'object' ? raw.ecommerce : null
  const fromFb =
    fallback?.ecommerce && typeof fallback.ecommerce === 'object'
      ? fallback.ecommerce
      : null
  ...
  const fbSettings =
    fromFb?.settings && typeof fromFb.settings === 'object'
      ? fromFb.settings
      : {}
  ...
  return {
    ...
    settings: {
      currency: ensureString(
        settingsSrc.currency,
        fbSettings.currency || 'USD',
      ),
...
function normalizeExportOptions(rawEx) {
  ...
  if (rawEx.embedSanityStudio != null)
    out.embedSanityStudio = Boolean(rawEx.embedSanityStudio)
...
function normalizePlanMeta(rawPm, fallbackPm) {
  ...
  return {
    ...
    contentRefStashName: ensureString(
      r.contentRefStashName,
      fb.contentRefStashName || '',
    ),
    archetypePresetKey: ensureString(
      r.archetypePresetKey,
      fb.archetypePresetKey || 'marketingDark',
    ),
    resolutionReason: ensureString(
      r.resolutionReason,
      fb.resolutionReason || '',
    ),
...
function normalizePage(page, idx = 0) {
  ...
  return {
    ...
    pageRole: ensureString(
      page?.pageRole,
      idx === 0 ? 'conversion' : 'standard',
    ),
    contentGoals:
        ...
        ? {
            ...
            bodyAttributes:
              ...
              typeof page.renderBlueprint.bodyAttributes === 'object'
                ? page.renderBlueprint.bodyAttributes
                : {},
            originalHtmlDocument: ensureString(
...
function readSectionBrandLogo(section) {
  ...
  if (logo.kind === 'remote' && typeof logo.src === 'string' && logo.src.trim())
    return logo
  if (logo.kind === 'svg' && typeof logo.svg === 'string' && logo.svg.trim())
    return logo
  return null
}
...
function enforceBrandConsistency(pages, projectName) {
  ...
  for (const page of pages) {
    for (const section of page?.sections || []) {
      ...
      if (brandLogo) {
        section.styling =
          section.styling && typeof section.styling === 'object'
            ? section.styling
            : {}
...
export function normalizeSiteSpec(input, context = {}) {
  ...
  const normalizedPages = pages.length
    ? pages
    : fallback.pages.map(normalizePage)
  ...
  return {
    ...
    generatedTimestamp: ensureString(
      raw.generatedTimestamp,
      fallback.generatedTimestamp,
    ),
    ...
    theme:
      raw.theme && typeof raw.theme === 'object'
        ? {
            ...
            spacing: {
              ...fallback.theme.spacing,
              ...(raw.theme.spacing || {}),
            },
            shadows: {
              ...fallback.theme.shadows,
              ...(raw.theme.shadows || {}),
            },
            appearance: {
              ...fallback.theme.appearance,
              ...(raw.theme.appearance || {}),
            },
            mood: ensureString(raw.theme.mood, fallback.theme.mood),
            tailwind: {
              ...fallback.theme.tailwind,
              ...(raw.theme.tailwind || {}),
            },
          }
        : fallback.theme,
    navigation:
      raw.navigation && typeof raw.navigation === 'object'
        ? {
            ...
            ctas: ensureArray(raw.navigation.ctas).map((item, idx) =>
              normalizeAction(item, idx),
            ),
          }
        : fallback.navigation,
    ...
    seo:
      raw.seo && typeof raw.seo === 'object'
        ? {
            ...
            ogImageAlt: ensureString(
              raw.seo.ogImageAlt,
              fallback.seo.ogImageAlt,
            ),
            twitterCard: ensureString(
              raw.seo.twitterCard,
              fallback.seo.twitterCard,
            ),
