export function buildHeuristicBusinessProfile({
  prompt = '',
  siteType = 'landing',
  ctx = {},
}) {
  ...
  let taxFootprint =
    'Follow applicable sales / VAT rules for your contracts; consult a tax advisor for cross-border B2B.'
  const trustSignals = []

  const mentionsEu =
    /\b(eu|europe|gdpr|berlin|amsterdam|paris|madrid|stockholm|gmbh|bv\b|ltd\.?\s*uk)\b/i.test(
      prompt,
    ) || /\b(european|eu-central)\b/i.test(prompt)
  if (mentionsEu) {
    jurisdiction = 'European Union'
    taxFootprint =
      'EU VAT / reverse charge for qualifying B2B; GDPR for personal data.'
    legalForm =
      legalForm ||
      'Private limited (example: BV / GmbH) — align with your registrar'
  }
  ...
  if (/\b(fintech|payments|banking|lending|neobank)\b/.test(lower)) {
    ...
    trustSignals.push(
      'Financial compliance posture varies by license — surface disclosures where required',
    )
    ...
    industryCode = {
      system: 'NACE',
      code: '62.01',
      label: 'Computer programming (health software)',
    }
    trustSignals.push(
      'Health data handling must match jurisdiction (e.g. HIPAA in US)',
    )
  } else if (
    st === 'ecommerce' ||
    /\b(dtc|direct.to.consumer|online store|shop|cart|checkout|retail)\b/.test(
      lower,
    )
  ) {
    ...
    st === 'institutional' ||
    /\b(government|public sector|ministry|municipal|psu)\b/.test(lower)
  ) {
    ...
    taxFootprint =
      'Not a consumer storefront; procurement and transparency rules apply'
    ...
    /\b(marketplace|platform|two-sided|buyers and sellers)\b/.test(lower)
  ) {
    ...
    industryCode = {
      system: 'NACE',
      code: '70.22',
      label: 'Business and management consultancy',
    }
    revenueModel = 'Time & materials, retainers, fixed projects'
  }

  if (/\b(enterprise|fortune|global 2000)\b/.test(lower)) segment = 'Enterprise'
  else if (/\b(smb|small business|startup|seed)\b/.test(lower))
    segment = 'SMB / startup'

  if (/\b(soc\s*2|soc2|iso\s*27001)\b/.test(lower))
    trustSignals.push('SOC2 / security program (as stated in prompt)')
  ...
  return {
    ...
    trustSignals: trustSignals.length
      ? trustSignals
      : ['Match copy to your actual certifications — do not invent badges'],
  }
}
...
export function normalizeBusinessProfile(raw, fallback) {
  ...
  const ic =
    raw.industryCode && typeof raw.industryCode === 'object'
      ? raw.industryCode
      : {}
  return {
    customerModel:
      typeof raw.customerModel === 'string' && raw.customerModel.trim()
        ? raw.customerModel
        : base.customerModel,
    industry:
    ...
    industryCode: {
      system:
        typeof ic.system === 'string' && ic.system.trim()
          ? ic.system
          : base.industryCode.system,
      code:
        typeof ic.code === 'string' && ic.code.trim()
          ? ic.code
          : base.industryCode.code,
      label:
        typeof ic.label === 'string' && ic.label.trim()
          ? ic.label
          : base.industryCode.label,
    },
    legalForm:
      typeof raw.legalForm === 'string' && raw.legalForm.trim()
        ? raw.legalForm
        : base.legalForm,
    jurisdiction:
    ...
    segment:
      typeof raw.segment === 'string' && raw.segment.trim()
        ...
        : base.segment,
    revenueModel:
      typeof raw.revenueModel === 'string' && raw.revenueModel.trim()
        ? raw.revenueModel
        : base.revenueModel,
    taxFootprint:
      typeof raw.taxFootprint === 'string' && raw.taxFootprint.trim()
        ? raw.taxFootprint
        : base.taxFootprint,
    trustSignals: Array.isArray(raw.trustSignals)
      ? raw.trustSignals.map((s) => String(s)).filter(Boolean)
      : base.trustSignals,
  }
}
