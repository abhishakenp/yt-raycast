...
describe('generated site PSEO guardrails', () => {
  it('marks query and hash routes noindex so generated duplicate states are not published as canonical pages', () => {
    ...
    expect(
      result.pages.find(
        (p: { route: string; seo?: unknown }) =>
          p.route === '/products?sort=popular',
      )?.seo,
    ).toMatchObject({ noIndex: true })
    expect(
      result.pages.find(
        (p: { route: string; seo?: unknown }) => p.route === '/pricing#faq',
      )?.seo,
    ).toMatchObject({ noIndex: true })
    expect(
      result.pages.find(
        (p: { route: string; seo?: unknown }) => p.route === '/about',
      )?.seo,
    ).toBeUndefined()
  })
  ...
  it('keeps the first twelve pages in a generated thin family indexable and noindexes later thin pages', () => {
    ...
    expect(
      result.pages.find(
        (p: { route: string; seo?: unknown }) =>
          p.route === '/locations/city-12',
      )?.seo,
    ).toBeUndefined()
    expect(
      result.pages.find(
        (p: { route: string; seo?: unknown }) =>
          p.route === '/locations/city-13',
      )?.seo,
    ).toMatchObject({ noIndex: true })
    expect(
      result.pages.find(
        (p: { route: string; seo?: unknown }) =>
          p.route === '/locations/city-14',
      )?.seo,
    ).toMatchObject({ noIndex: true })
  })
  ...
  it('leaves later generated family pages indexable when they carry substantive unique descriptions', () => {
    ...
    expect(
      result.pages.find(
        (p: { route: string; seo?: unknown }) => p.route === '/clinics/city-13',
      )?.seo,
    ).toBeUndefined()
    expect(
      result.pages.find(
        (p: { route: string; seo?: unknown }) => p.route === '/clinics/city-14',
      )?.seo,
    ).toBeUndefined()
  })
  ...
  it('adds missing footer links for indexable generated pages without duplicating already-linked routes', () => {
    ...
    expect(
      footerHrefs.filter((href: string) => href === '/about'),
    ).toHaveLength(0)
    expect(
      footerHrefs.filter((href: string) => href === '/services'),
    ).toHaveLength(1)
