...
function applyLangToDocument(html: string, htmlLang: string): string {
  if (!htmlLang || !/<html\b/i.test(html)) return html
  return html.replace(/<html\b([^>]*)>/i, (_match: string, attrs: string) => {
...
function applySeoToExactCloneDocument(
  html: string,
  siteSpec: any,
  page: any,
): string {
  ...
  if (/<head\b[^>]*>/i.test(withLang)) {
    return withLang.replace(
      /<head\b[^>]*>([\s\S]*?)<\/head>/i,
      (_match: string, headInner: string) => {
...
function applySiteCssHrefToHtml(html: string, siteCssHref: string): string {
...
function renderPageDocument(
  ...
  page: any,
  siteCssHref: string,
  siteMotionHref: string,
  useSwiper = false,
): string {
  const sections = (page.sections || [])
    .map((section: any) => renderSectionHtml(section, siteSpec))
    .join('\n')
  const { htmlLang, markup } = renderSeoHeadMarkup(siteSpec, page)
...
export function renderHtmlProject(siteSpec: any) {
    ...
    for (const page of siteSpec.pages || []) {
      for (const section of page.sections || []) {
        if (
          ...
          section.items = siteSpec.ecommerce.products.map((p: any) => ({
            id: p.id,
            title: p.title,
            body: p.description,
            price:
        ...
        if (
          ...
          section.items = siteSpec.ecommerce.products
            .slice(0, 4)
            .map((p: any) => ({
              id: p.id,
              title: p.title,
