...
type SiteSpec = SiteSpecLike & {
  theme?: Theme | string | Record<string, unknown>
  pages?: Array<Page | Record<string, unknown>>
  [key: string]: unknown
}

type Page = SitePageLike & {
  id?: string
  [key: string]: unknown
}
...
interface Theme {
  ...
  colors?: { background?: string; [key: string]: unknown }
  [key: string]: unknown
}

function collectThemeGoogleFontFamilies(theme: Theme = {}): string[] {
...
function buildNextLayoutFontLinkLines(siteSpec: SiteSpec): string {
  const theme = siteSpec.theme
  const themeObj =
    typeof theme === 'object' && theme !== null ? (theme as Theme) : undefined
  const families = collectThemeGoogleFontFamilies(themeObj)
...
function renderMedusaExportFiles(session: Session): Record<string, string> {
  const backendUrl =
...
function renderNextSectionRenderer(siteSpec: SiteSpec): string {
...
export function renderNextProject(
  siteSpec: SiteSpec,
  session: Session,
