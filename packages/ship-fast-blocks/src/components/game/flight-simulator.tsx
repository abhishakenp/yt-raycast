...
export const flightSimulatorProps = z.object({
  // Player aircraft
  ...
  skyColorHex: cssColor
    .optional()
    .describe("Sky color — only a CSS color value, e.g. '#1a1a2e' or 'orange'."),
  grassColorHex: cssColor
    .optional()
    .describe("Ground terrain color — only a CSS color value, e.g. '#2d5a27' or 'green'."),
  fogColorHex: cssColor
    .optional()
    .describe("Fog color — only a CSS color value, e.g. '#cccccc' or 'white'."),
  waterColorHex: cssColor
    .optional()
    .describe("Water/pond color — only a CSS color value, e.g. '#0077be' or 'cyan'."),
  buildingColorHex: cssColor
    .optional()
    .describe("Building color — only a CSS color value, e.g. '#ff6b35' or 'silver'."),
  robotColorHex: cssColor
    .optional()
    .describe("Robot landmark color — only a CSS color value, e.g. '#ff0000' or 'red'."),
  castleColorHex: cssColor
    .optional()
    .describe("Castle stone color — only a CSS color value, e.g. '#8b7355' or 'gray'."),
  ...
  buildingHeight: z
    .number()
    .optional()
    .describe("Max building height in units (default 400, increase for taller buildings)"),
  treeCount: z
    .number()
    .optional()
    .describe("Number of trees (default 7000)"),
  cloudCount: z
    .number()
    .optional()
...
export type FlightSimulatorProps = z.infer<typeof flightSimulatorProps>
...
export default function FlightSimulator(cfg: FlightSimulatorProps) {
  ...
  useEffect(() => {
    ...
    let onKeyUp: ((e: KeyboardEvent) => void) | undefined; let onResize: (() => void) | undefined;
    ...
    (async () => {
      try {
        ...
        const controls: Record<string, number> = {
          ...
          up: 0, down: 0, boost: 0, rollLeft: 0, rollRight: 0,
        };
        ...
        function init() {
          ...
          camera = new THREE.PerspectiveCamera(
            CAMERA_BASE_FOV, window.innerWidth / window.innerHeight, 0.1, 20000,
          );
          ...
          new THREE.TextureLoader().load(
            ...
            (texture) => {
              ...
              const groundMaterial = new THREE.MeshStandardMaterial({
                map: texture, side: THREE.DoubleSide, roughness: 0.9, metalness: 0.1,
              });
            ...
            () => {
              ...
              const m = new THREE.MeshStandardMaterial({ color: 0x555555, side: THREE.DoubleSide });
        ...
        function createSkyscrapers(count: number) {
          const mat = new THREE.MeshStandardMaterial({ color: 0xaaaaaa, roughness: 0.8, metalness: 0.2 });
        ...
        function createHouses(clusterCount: number) {
          ...
          for (let c = 0; c < clusterCount; c++) {
            ...
            for (let i = 0; i < count; i++) {
              ...
              const houseMat = new THREE.MeshStandardMaterial({ color: 0xcc6633, roughness: 0.9 });
        ...
        function createPonds(count: number) {
          const pondMat = new THREE.MeshStandardMaterial({
            color: 0x3366aa, roughness: 0.2, metalness: 0.1, transparent: true, opacity: 0.8,
          });
          for (let i = 0; i < count; i++) {
            ...
            const pond = new THREE.Mesh(new THREE.CircleGeometry(radius, 32), pondMat);
        ...
        function createCastle() {
          ...
          const keep = new THREE.Mesh(new THREE.BoxGeometry(40, 60, 40), stoneMat);
          keep.position.y = 30;
          ...
          const banner = new THREE.Mesh(
            ...
            new THREE.MeshBasicMaterial({ map: new THREE.CanvasTexture(canvas), side: THREE.DoubleSide, transparent: true, opacity: 0.9, fog: false }),
          );
          banner.position.set(0, 40, 20.1);
          castleGroup.add(banner);

          const towerGeo = new THREE.CylinderGeometry(6, 8, 70, 8);
          const towerPositions: [number, number, number][] = [[-22, 35, -22], [22, 35, -22], [-22, 35, 22], [22, 35, 22]];
          for (const pos of towerPositions) {
            const tower = new THREE.Mesh(towerGeo, stoneMat);
            tower.position.set(pos[0], pos[1], pos[2]);
            castleGroup.add(tower);
            const roof = new THREE.Mesh(new THREE.ConeGeometry(8, 15, 8), new THREE.MeshPhongMaterial({ color: 0x8b4513 }));
          ...
          for (let x = -18; x <= 18; x += 4) {
            for (let z = -18; z <= 18; z += 4) {
              if (x === -18 || x === 18 || z === -18 || z === 18) {
                const merlon = new THREE.Mesh(new THREE.BoxGeometry(3, 4, 3), stoneMat);
        ...
        function createBigHouse(count: number) {
          for (let i = 0; i < count; i++) {
            ...
            const roof = new THREE.Mesh(
              ...
              new THREE.MeshPhongMaterial({ color: Math.random() > 0.5 ? 0x8b4513 : 0xa0522d }),
            );
            ...
            const winMat = new THREE.MeshPhongMaterial({ color: 0xaaaaff, emissive: 0x444444 });
            const winPositions = [[-w / 4, h / 2, d / 2], [w / 4, h / 2, d / 2], [-w / 4, h * 0.75, d / 2], [w / 4, h * 0.75, d / 2]];
        ...
        function createGiantRobot() {
          ...
          const bodyMat = new THREE.MeshPhongMaterial({ color: 0xb894db, shininess: 70, fog: false });
          const body = new THREE.Mesh(new THREE.BoxGeometry(10, 14, 6), bodyMat);
          robotGroup.add(body);

          const headMat = new THREE.MeshPhongMaterial({ color: 0x935dc9, shininess: 70, fog: false });
          const head = new THREE.Mesh(new THREE.BoxGeometry(8, 6, 6), headMat);
          head.position.y = 10;
          robotGroup.add(head);

          const eyeMat = new THREE.MeshPhongMaterial({ color: 0xff0000, emissive: 0xff0000, emissiveIntensity: 0.8, fog: false });
          ...
          const legMat = new THREE.MeshPhongMaterial({ color: 0x935dc9, shininess: 70, fog: false });
          ...
          bCtx.fillText("NEO.AI", bannerCanvas.width / 2, bannerCanvas.height / 2);
          const banner = new THREE.Mesh(
            new THREE.PlaneGeometry(16, 4),
            new THREE.MeshBasicMaterial({ map: new THREE.CanvasTexture(bannerCanvas), transparent: true, side: THREE.DoubleSide, fog: false }),
          );
        ...
        function createMountain() {
          ...
          const mountainMat = new THREE.MeshPhongMaterial({ color: 0x8b4513, flatShading: true });
          const mountain = new THREE.Mesh(new THREE.ConeGeometry(100, 200, 6), mountainMat);
          ...
          sCtx.fillText("FLIGHT SIMULATOR", signCanvas.width / 2, signCanvas.height / 2);
          ...
          const snowCap = new THREE.Mesh(new THREE.ConeGeometry(40, 50, 6), snowMat);
          snowCap.position.y = 175;
          mountainGroup.add(snowCap);

          const smallPeaks: [number, number, number][] = [[-50, -30, -40], [60, -50, 30], [-30, -60, 50]];
          for (const pos of smallPeaks) {
            const h = 80 + Math.random() * 60;
            const peak = new THREE.Mesh(new THREE.ConeGeometry(30 + Math.random() * 20, h, 5), mountainMat);
            peak.position.set(pos[0], h / 2, pos[1]);
            peak.rotation.y = Math.random() * Math.PI;
            mountainGroup.add(peak);
            if (Math.random() > 0.3) {
              const snow = new THREE.Mesh(new THREE.ConeGeometry(15, 20, 5), snowMat);
          ...
          for (let i = 0; i < 50; i++) {
            ...
            tree.add(new THREE.Mesh(new THREE.CylinderGeometry(2, 3, h, 8), new THREE.MeshPhongMaterial({ color: 0x8b4513 })));
            const top = new THREE.Mesh(new THREE.ConeGeometry(8, h * 1.5, 8), new THREE.MeshPhongMaterial({ color: 0x006400, flatShading: true }));
        ...
        function createUFO() {
          ...
          const bodyMat = new THREE.MeshPhongMaterial({ color: 0xc0c0c0, shininess: 80, fog: false });

          const top = new THREE.Mesh(
            new THREE.SphereGeometry(10, 32, 16, 0, Math.PI * 2, 0, Math.PI / 2), bodyMat,
          );
          top.scale.y = 0.3;
          ufoGroup.add(top);

          const bottom = new THREE.Mesh(
            new THREE.SphereGeometry(7, 32, 16, 0, Math.PI * 2, Math.PI / 2, Math.PI / 4), bodyMat,
          );
          bottom.scale.y = 0.5;
          ufoGroup.add(bottom);
          ...
          const cockpit = new THREE.Mesh(
            ...
            new THREE.MeshPhongMaterial({ transparent: true, opacity: 0.9, shininess: 100, fog: false }),
          );
          ...
          const plate = new THREE.Mesh(
            ...
            new THREE.MeshPhongMaterial({ color: 0xc0c0c0, shininess: 60, side: THREE.DoubleSide, fog: false }),
          );
          plate.rotation.x = Math.PI / 2;
          ufoGroup.add(plate);

          const lightPositions: [number, number, number][] = [[5, -2, 0], [-5, -2, 0], [0, -2, 5], [0, -2, -5]];
          const lightMat = new THREE.MeshBasicMaterial({ color: 0x00ffff, fog: false });
        ...
        function createBlimp() {
          const makeBlimp = (color: number) => {
            ...
            group.add(new THREE.Mesh(bodyGeo, new THREE.MeshPhongMaterial({ color })));
            const tailGeo = new THREE.ConeGeometry(15, 20, 32);
            tailGeo.rotateX(Math.PI / 2);
            tailGeo.translate(0, 0, 45);
            group.add(new THREE.Mesh(tailGeo, new THREE.MeshPhongMaterial({ color: 0x00008b })));
            const gondola = new THREE.Mesh(new THREE.BoxGeometry(10, 5, 7), new THREE.MeshPhongMaterial({ color: 0x404040 }));
        ...
        function createPlayerAircraft() {
          ...
          const glslVec3FromHex = (hex: string, mult = 1) => {
            const c = hex.replace("#", "");
            const f = (i: number) => Math.min(255, parseInt(c.slice(i, i + 2), 16) * mult) / 255;
        ...
        function createTrees(count: number) {
          const trunkGeo = new THREE.CylinderGeometry(TRUNK_RADIUS, TRUNK_RADIUS, TRUNK_HEIGHT, 8);
          const trunkMat = new THREE.MeshStandardMaterial({ color: 0x8b4513, roughness: 0.9 });
          const foliageGeo = new THREE.ConeGeometry(FOLIAGE_RADIUS, FOLIAGE_HEIGHT, 8);
          const foliageMat = new THREE.MeshStandardMaterial({ color: 0x228b22, roughness: 0.9 });
          ...
          for (let i = 0; i < count; i++) {
            ...
            for (const bb of buildingBoundingBoxes) {
              if (treeBox.intersectsBox(bb)) { hit = true; break; }
            }
            if (hit) continue;
        ...
        function createClouds() {
          new THREE.TextureLoader().load(CLOUD_TEXTURE_URL, (cloudTexture: THREE.Texture) => {
            if (disposed) return;
            const geo = new THREE.BufferGeometry();
            ...
            for (let i = 0; i < CLOUD_COUNT; i++) {
              positions.push(
                ...
                Math.random() * (CLOUD_ALTITUDE_MAX - CLOUD_ALTITUDE_MIN) + CLOUD_ALTITUDE_MIN,
                (Math.random() - 0.5) * CLOUD_AREA_XZ,
              );
            }
            geo.setAttribute("position", new THREE.Float32BufferAttribute(positions, 3));
            clouds = new THREE.Points(geo, new THREE.PointsMaterial({
              size: CLOUD_SIZE, map: cloudTexture, blending: THREE.AdditiveBlending,
              depthWrite: false, transparent: true, opacity: 0.6, sizeAttenuation: true, color: 0xeeeeff,
            }));
            scene.add(clouds);
          });
        }
        ...
        function createRoads() {
          ...
          const roadMat = new THREE.MeshStandardMaterial({
            map: roadTexture, side: THREE.FrontSide, roughness: 0.9, metalness: 0.0,
          ...
          for (let i = 0; i < ROAD_COUNT; i++) {
            ...
            const angle = Math.random() * Math.PI * 2;
            const len = ROAD_MIN_LENGTH + Math.random() * (ROAD_MAX_LENGTH - ROAD_MIN_LENGTH);
        ...
        function setupControls() {
          onKeyDown = (e: KeyboardEvent) => {
            switch (e.code) {
              ...
              case "KeyS": case "ArrowDown": controls.backward = 1; break;
              case "KeyA": case "ArrowLeft": controls.left = 1; break;
              case "KeyD": case "ArrowRight": controls.right = 1; break;
              ...
              case "ControlLeft": case "ControlRight": controls.down = 1; break;
              case "Space": controls.up = 1; break;
            }
          };
        ...
        function animate() {
          ...
          if (clouds) {
            ...
            if (clouds.position.x > CLOUD_AREA_XZ / 2) clouds.position.x -= CLOUD_AREA_XZ;
          }
          ...
          if (playerAircraft) {
            ...
            const camOffset = relOffset.applyQuaternion(playerAircraft.quaternion);
            const desired = playerAircraft.position.clone().add(camOffset);
            camera.position.lerp(desired, 0.1);
            camera.lookAt(playerAircraft.position.clone().add(new THREE.Vector3(0, 1, 0)));
          }
        ...
        function updatePlayerMovement(dt: number) {
          ...
          const maxSpeed = PLAYER_SPEED * (controls.boost ? AFTERBURNER_MULTIPLIER : 1);
          ...
          playerAngularVelocity.x += (targetPitch - playerAngularVelocity.x) * dt * 5.0;
          playerAngularVelocity.y += (targetYaw - playerAngularVelocity.y) * dt * 5.0;
          playerAngularVelocity.z += (targetRoll - playerAngularVelocity.z) * dt * 5.0;
          playerAngularVelocity.multiplyScalar(DAMPING);

          const dRot = playerAngularVelocity.clone().multiplyScalar(dt);
          const qx = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1, 0, 0), dRot.x);
          const qy = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), dRot.y);
          const qz = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 0, 1), dRot.z);
          playerAircraft.quaternion.multiply(qx).multiply(qy).multiply(qz).normalize();

          const fwd = new THREE.Vector3(0, 0, -1).applyQuaternion(playerAircraft.quaternion);
        ...
        function updateCameraFOV() {
          const ratio = playerVelocity.length() / (PLAYER_SPEED * AFTERBURNER_MULTIPLIER);
        ...
        function checkCollisions() {
          if (!playerAircraft || !playerAircraft.children || playerAircraft.children.length === 0)
            return false;
            ...
            if ((mesh as THREE.Mesh).isMesh && mesh.material) {
              ...
              for (const mat of mats) {
                if (
                  ...
                  (mat as THREE.MeshStandardMaterial).emissive.setHex(isColliding ? 0xaa0000 : 0x000000);
                }
              }
            }
          });
        ...
        function updateInfoPanel() {
          if (speedEl) speedEl.textContent = (playerVelocity.length() * 3.6).toFixed(0);
          if (altEl && playerAircraft) altEl.textContent = playerAircraft.position.y.toFixed(1);
        }

        // --- Start ---
        init();
      } catch (err) {
        if (!disposed) setError(err instanceof Error ? err.message : "Failed to initialize game");
      }
    })();
    ...
    return () => {
      ...
      if (rendererInstance?.domElement && container.contains(rendererInstance.domElement)) {
