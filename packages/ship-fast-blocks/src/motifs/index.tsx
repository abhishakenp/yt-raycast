...
import { SiteNav, type SiteNavProps } from '#/section-kit/SiteNav.tsx'
import {
  SiteFooter,
  FooterContent,
  FooterGrid,
...
function useChrome(
  ...
  return propsChrome ?? (ctxChrome as ChromeVariant | undefined) ?? fallback
}
...
function useDecor(propsDecor: DecorVariant | undefined): DecorVariant {
  ...
  return propsDecor ?? (ctxDecor as DecorVariant | undefined) ?? 'none'
}
...
function ArtisticPlaceholderPanel({
  ...
  if (chrome === 'terminal') {
    return (
      <div
        data-d-role="card"
        className="d-border-lock relative border border-border bg-card"
      >
        <div
  ...
  if (chrome === 'brutalist') {
    return (
      <div
        data-d-role="card"
        className="d-border-lock relative aspect-square border-2 border-foreground bg-muted/40 shadow-[8px_8px_0_0] shadow-foreground"
      >
        <div
  ...
  if (chrome === 'hairline') {
    return (
      <div
        data-d-role="card"
        className="d-border-lock relative aspect-square border border-border bg-card"
      >
        <div
          className="flex h-full items-center justify-center"
          aria-hidden="true"
          ...
          <div className="grid items-center gap-12 lg:grid-cols-12 lg:gap-10">
            <div className="lg:col-span-8">
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:gap-4">
                ...
                <span
                  data-d-role="btn"
                  className="d-border-lock inline-flex items-center justify-center rounded-none border border-foreground px-7 py-3.5 text-sm font-medium tracking-wide text-foreground transition-colors duration-150 hover:bg-foreground hover:text-background active:translate-y-px"
                >
                  {secondaryCta}
                </span>
              </div>
          ...
          <div className={cn('relative lg:col-span-5 lg:pl-14')}>
            {imageAlt ? (
              chrome === 'terminal' ? (
                ...
                <HeroMediaPanel alt={imageAlt} className="aspect-square" />
              )
            ) : (
          ...
          >
            {cards.map((card, i) => (
              <div
                key={i}
                data-d-role="card"
                className="d-border-lock group relative overflow-hidden border-b border-r border-border p-6 sm:p-8"
              >
                {/* Giant ghost numeral */}
                <span
                  aria-hidden="true"
            ...
            {cells.map((cell, i) => (
              <BentoTile
                ...
                className={cn(
                  ...
                  chrome === 'hairline' || chrome === 'terminal'
                    ? 'd-border-lock rounded-none border border-border bg-card transition-colors duration-150 hover:bg-muted/30'
                    : 'd-border-lock d-radius-lock rounded-none border-2 border-foreground bg-background',
                )}
              >
          ...
          >
            {testimonials.map((t, i) => (
              <div
                ...
                data-d-role="card"
                className={cn(
                  'd-border-lock group flex flex-col gap-6 rounded-none border border-border bg-background p-6 transition-colors duration-200 hover:border-foreground/40 sm:p-8',
              ...
              return (
                <PricingTier
                  ...
                  className={cn(
                    chrome === 'hairline' || chrome === 'terminal'
                      ? cn(
                          'd-border-lock d-shadow-lock rounded-none border-0 border-b border-r border-border shadow-none',
                          highlighted && 'bg-foreground text-background ring-0',
                        )
                      : chrome === 'brutalist'
                        ? cn(
                            'd-border-lock d-radius-lock rounded-none border-2 border-foreground',
                            highlighted &&
                    ...
                    <PricingTierCta
                      className={cn(
                        highlighted
                          ...
                          : chrome === 'brutalist'
                            ? 'd-border-lock border-2 border-foreground bg-background text-foreground hover:bg-muted'
                            : 'd-border-lock border border-border bg-background text-foreground hover:bg-muted',
                        chrome === 'hairline' || chrome === 'terminal'
                          ? 'rounded-none'
                          : '',
                      )}
          ...
          <div className="grid grid-cols-1 gap-0 border-l border-t border-border sm:grid-cols-3">
            {steps.map((step, i) => (
              <div
                key={i}
                data-d-role="card"
                className="d-border-lock relative overflow-hidden border-b border-r border-border p-6 sm:p-8"
              >
                {/* Giant ghost ordinal */}
                <span
                  aria-hidden="true"
          ...
          <div className="mt-12 flex flex-col gap-0 border-l border-t border-border">
            {events.map((event, i) => (
              <div
                data-d-role="card"
                key={i}
                className="d-border-lock relative border-b border-r border-border bg-card p-6"
              >
                <GhostNumeral numeral={String(i + 1).padStart(2, '0')} />
                <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-primary">
                  {chrome === 'terminal' && (
    ...
    return (
      ...
      >
        ...
        >
          ...
          <button
            data-d-role="btn"
            className="d-border-lock mt-2 border-2 border-background bg-background px-8 py-3 font-bold uppercase tracking-wider text-foreground transition-all hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[6px_6px_0_0] hover:shadow-background"
          >
            {cta}
          </button>
        </Container>
    ...
    return (
      <P.Section className="border-y border-border bg-muted/40 py-16">
        ...
        >
          ...
          <button
            data-d-role="btn"
            className="mt-2 bg-primary px-8 py-3 font-mono text-sm font-bold uppercase tracking-wider text-primary-foreground transition-colors hover:bg-primary/90"
          >
            {cta}
          </button>
        </Container>
    ...
    return (
      ...
      >
        ...
        >
          ...
          <button
            data-d-role="btn"
            className="d-border-lock mt-2 border border-background/30 bg-transparent px-8 py-3 text-sm font-medium tracking-wide text-background transition-all hover:border-background hover:bg-background hover:text-foreground"
          >
            {cta}
          </button>
        </Container>
        ...
        >
          ...
          <form className="flex w-full max-w-md flex-col gap-3 sm:flex-row">
            ...
            <button
              type="submit"
              data-d-role="btn"
              className="d-border-lock whitespace-nowrap border-2 border-background bg-foreground px-8 py-3.5 text-sm font-bold uppercase tracking-wider text-background transition-all hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[6px_6px_0_0] hover:shadow-background"
            >
              {cta}
            </button>
          </form>
  ...
  return (
    <P.Section>
      ...
      >
        ...
        <div
          data-d-role="card"
          className="d-border-lock relative aspect-video overflow-hidden border border-border bg-muted"
        >
          <P.ImageBlock
            alt={imageAlt}
            src={props.imageSrc as string | undefined}
          ...
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/90 ring-4 ring-background/50">
              <svg
                viewBox="0 0 24 24"
                fill="none"
          ...
          <div className="flex gap-1 border-b border-border">
            {tabs.map((tab, i) => (
              <button
                key={i}
                data-d-role="btn"
                className="d-border-lock border-b-2 border-transparent px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground"
              >
                {tab.label}
              </button>
            ))}
        ...
        <div className="flex gap-2">
          {filters.map((f, i) => (
            <button
              key={i}
              data-d-role="btn"
              className="d-border-lock border border-border px-3 py-1 text-xs font-medium text-muted-foreground hover:bg-muted"
            >
              {f}
            </button>
          ))}
