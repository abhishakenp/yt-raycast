...
const JOB_TAG_SEPARATOR = " | "

const parseJobTags = (rawTags: string) =>
  rawTags
    .split(rawTags.includes(JOB_TAG_SEPARATOR) ? JOB_TAG_SEPARATOR : ",")
    .map((tag) => tag.trim())
    .filter(Boolean)

const normalizeJobTags = (tags: string[]) => tags.join(JOB_TAG_SEPARATOR)
 ...
 */
export const JobBoardKimiPage = defineCapsule({
  ...
  lakebed: {
    ...
    mutations: {
      applyToJob: ({ db }, role: string, company: string, logoAlt: string, tags: string) => {
        const exists = db.applications.where('role', role).all().some((application) => application.company === company)
        if (exists) {
      ...
      removeApplication: ({ db }, applicationId: string) => {
        db.applications.delete(applicationId)

        return db.applications.all()
      },
      clearApplications: ({ db }) => {
        for (const application of db.applications.all()) {
          db.applications.delete(application.id)
        }
  ...
  component: ({ props, lakebed }) => {
    ...
    const defaultJobs = props.jobs?.items?.length
      ? props.jobs.items
      : [
          {
    ...
    const persistedJobs = storedJobs && storedJobs.length > 0 ? storedJobs : defaultJobs.map((job) => ({ ...job, tags: normalizeJobTags(job.tags) }))
    const jobItems = persistedJobs.map((job) => ({
      role: job.role,
      company: job.company,
      logoAlt: job.logoAlt,
            ...
            <div className="flex h-16 items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <button
                  ...
                  className="relative hidden items-center justify-center rounded-lg border border-border bg-background px-3 py-2 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground sm:inline-flex"
                ...
                >
                  <svg
                    ...
                    aria-hidden="true"
                  >
                    <path d="M3 3h18a2 2 0 0 1 2 2v16l-5-3-5 3-5-3-5 3V5a2 2 0 0 1 2-2z" />
                  </svg>
                  {applicationCount > 0 ? (
                    <span className="absolute -right-2 -top-2 grid size-4 place-items-center rounded-full bg-primary px-1 text-[0.625rem] font-bold text-primary-foreground">
                      {applicationCount}
                    </span>
                  ) : null}
                </button>
                          ...
                          onClick={() => {
                            if (isSignedIn) {
                              void applyToJob(
                                ...
                                job.company,
                                job.logoAlt,
                                normalizeJobTags(job.tags),
                              )
                              setApplicationsOpen(true)
                            }

                            go(`${jobsApplyLabel}: ${job.role}`)
                          }}
        ...
        <Sheet open={applicationsOpen} onOpenChange={setApplicationsOpen}>
          <SheetContent side="right" className="w-full gap-0 p-0 sm:max-w-md">
            <SheetHeader className="border-b border-border p-6">
              <SheetTitle className="text-xl">Saved applications</SheetTitle>
              <SheetDescription>
                {applications.length
                  ? `${applications.length} application${applications.length === 1 ? '' : 's'} ready for follow-up.`
                  : 'No applications saved in this session yet.'}
              </SheetDescription>
            </SheetHeader>
              ...
              {applications.length ? (
                  ...
                  {applications.map((application) => (
                    ...
                    >
                      ...
                      <div className="mb-3 flex flex-wrap gap-2">
                        {parseJobTags(application.tags).map((tag) => (
                          <span
                            key={tag}
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            go(`${jobsApplyLabel}: ${application.role}`)
                ...
                <div className="flex min-h-64 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-muted/40 px-6 text-center">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Save jobs to review them and keep your hiring pipeline organized.
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="rounded-lg border border-input px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-muted disabled:pointer-events-none disabled:opacity-60"
                  >
                    Clear all
                  </button>
                  <SheetClose asChild>
