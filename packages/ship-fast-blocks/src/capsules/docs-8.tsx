...
export const DocsKimiPage8 = defineCapsule({
  name: "DocsKimiPage8",
  description:
    "Docs 8th style sibling to DocsKimiPage, converted from generated Kimi HTML into a responsive token-compliant page block with hero storytelling, metrics, content sections, image-led cards, and conversion actions.",
  props: z.object({
  ...
  lakebed: {
    ...
    mutations: {
      addBookmark: ({ db }, sectionTitle: string, sectionEyebrow: string) => {
        ...
        if (existing) {
          db.bookmarks.update(existing.id, { read: (existing.read || 0) + 1 })
        } else {
          db.bookmarks.insert({ sectionTitle, sectionEyebrow, read: 1 })
        }
  ...
  component: ({ props, lakebed }) => {
    ...
    const nav = props.nav?.length ? props.nav : ["Guide", "API", "Examples", "Help"]
    const hero = {
      eyebrow: "Docs / Variant 8",
      title: "Documentation",
      description: "Developer Documentation GreenDocs Guide API Examples Help Documentation Everything you need to integrate with our platform Onboarding Welcome Installation Configuration First Pr...",
      primaryCta: "Guide",
      secondaryCta: "API",
  ...
  {
    "eyebrow": "Overview",
    "title": "Welcome to GreenDocs",
    "body": "Developer Documentation GreenDocs Guide API Examples Help Documentation Everything you need to integrate with our platform Onboarding Welcome Installation Configuration First Pr...",
    "items": [
      "New to GreenDocs?",
      "Prerequisites",
      "Installation"
  ...
  },
  {
    ...
    const bookmarkedSectionTitles = new Set(bookmarks?.map((b) => b.sectionTitle) ?? [])
              ...
              <Sheet open={bookmarksOpen} onOpenChange={setBookmarksOpen}>
                ...
                <SheetContent side="right" className="w-full gap-0 p-0 sm:max-w-md">
                  <SheetHeader className="border-b border-border p-6">
                    ...
                    <SheetDescription>
                      {bookmarks && bookmarks.length > 0
                        ? `${bookmarks.length} section${bookmarks.length === 1 ? '' : 's'} saved.`
                    ...
                    {bookmarks && bookmarks.length ? (
                          ...
                          >
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="mt-1 text-xs text-muted-foreground">
                                Read {bookmark.read} time{bookmark.read === 1 ? '' : 's'}
                              </p>
                            </div>
                            <button
                              type="button"
                      ...
                      <div className="flex min-h-64 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-muted/40 px-6 text-center">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                          Bookmark sections to build your reading list.
                        </p>
                      </div>
                    )}
                  </div>
          ...
          <section className="border-y border-border bg-muted/40">
            <div className="mx-auto grid max-w-7xl gap-5 px-5 py-14 md:grid-cols-2">
              {sections.map((section, index) => {
                const isBookmarked = bookmarkedSectionTitles.has(section.title)
                return (
                  <article key={section.title} className="rounded-lg border border-border bg-card p-6">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (isBookmarked) {
                            void removeBookmark(section.title)
                          } else {
                            void addBookmark(section.title, section.eyebrow)
                          }
                        }}
                        ...
                        className="grid size-8 place-items-center rounded-full border border-border bg-background text-muted-foreground transition-all hover:bg-muted hover:text-foreground"
                      >
