...
export const EventKimiPage2 = defineCapsule({
  ...
  lakebed: {
    ...
    queries: {
      ...
      registeredSessions: ({ db }) =>
        db.registrations.all().flatMap((item) => {
          const session = db.sessions.get(item.sessionId)
          return session ? [{ ...item, session }] : []
        }),
      favoriteSessionTitles: ({ db }) =>
        new Set(db.favorites.all().map((favorite) => favorite.sessionTitle)),
    },
    mutations: {
      registerForSession: ({ db }, sessionTitle: string) => {
        const session = db.sessions.where('title', sessionTitle).all()[0]
        if (!session) return db.registrations.all()

        const existingRegistration = db.registrations
      ...
      toggleFavorite: ({ db }, sessionTitle: string) => {
        ...
        db.favorites.insert({ sessionTitle })
        return true
      },
    },
  },
  ...
  },
  {
    ...
    const favoriteSessionTitles = lakebed.useQuery('favoriteSessionTitles')
    const auth = lakebed.useAuth()
    const registerForSession = lakebed.useMutation('registerForSession')
    const unregisterFromSession = lakebed.useMutation('unregisterFromSession')
    ...
    const displaySessions =
      storedSessions && storedSessions.length > 0
        ? storedSessions
        : defaultSessions
    const safeRegisteredSessions = registeredSessions ?? []
    const registeredCount = safeRegisteredSessions.length
              ...
              <Sheet open={scheduleOpen} onOpenChange={setScheduleOpen}>
                ...
                >
                  <SheetHeader className="border-b border-border p-6">
                    <SheetTitle className="text-xl">My Schedule</SheetTitle>
                    <SheetDescription>
                      {registeredCount > 0
                        ? `${registeredCount} session${registeredCount === 1 ? '' : 's'} in your schedule.`
                        : 'Your schedule is empty.'}
                    </SheetDescription>
                  </SheetHeader>
                  <div className="flex-1 overflow-y-auto px-6 py-5">
                    {safeRegisteredSessions.length ? (
                      <div className="space-y-4">
                        {safeRegisteredSessions.map((item) => (
                          <div
                            key={item.id}
                  ...
                  >
                    <div className="p-5">
                      <div className="mb-3 flex items-start justify-between gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => void toggleFavorite(session.title)}
                      ...
                      >
                        {isRegistered ? 'Registered' : 'Add to Schedule'}
                      </Button>
                    </div>
                  </article>
                )
