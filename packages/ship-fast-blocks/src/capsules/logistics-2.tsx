...
export const LogisticsKimiPage2 = defineCapsule({
  ...
  lakebed: {
    ...
    mutations: {
      ...
      toggleSavedQuote: ({ db }, quoteId: string) => {
      ...
      removeQuote: ({ db }, quoteId: string) => {
        for (const quote of db.quotes.where('id', quoteId).all()) {
                ...
                <Sheet open={quotesOpen} onOpenChange={setQuotesOpen}>
                  ...
                  >
                    <SheetHeader className="border-b border-border p-6">
                      <SheetTitle className="text-xl">Saved Quotes</SheetTitle>
                      <SheetDescription>
                        {quoteCount > 0
                          ? `${quoteCount} quote${quoteCount === 1 ? '' : 's'} saved for this session.`
                          : 'No quotes saved yet.'}
                      </SheetDescription>
                    </SheetHeader>
                    ...
                    <SheetFooter className="border-t border-border p-6">
                      <button
                        ...
                        className="w-full rounded-full bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90 disabled:pointer-events-none disabled:opacity-60"
                        onClick={() => go('Checkout')}
                      >
                        Compare Quotes
                      </button>
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          ...
                          disabled={!safeQuotes.length}
                          className="rounded-full border-2 border-input px-4 py-3 text-sm font-semibold text-foreground transition-colors hover:border-primary hover:text-primary disabled:pointer-events-none disabled:opacity-60"
                        >
                          Clear All
                        </button>
                        <SheetClose asChild>
                          <button
                            type="button"
                            className="rounded-full bg-secondary px-4 py-3 text-sm font-semibold text-secondary-foreground transition-colors hover:bg-secondary/80"
                          >
                            Continue
                          </button>
                        </SheetClose>
                  ...
                  {isSignedIn ? (
                    <div className="space-y-3">
                      ...
                      <button
                        ...
                        className="w-full rounded-full bg-foreground px-4 py-2 text-sm font-semibold text-background transition-colors hover:bg-foreground/90"
                      >
                        Sign out
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      disabled={auth.isLoading}
                      className="flex w-full items-center justify-center gap-2 rounded-full bg-foreground px-4 py-2 text-sm font-semibold text-background transition-colors hover:bg-foreground/90 disabled:pointer-events-none disabled:opacity-60"
                    >
                      <span className="grid size-5 place-items-center rounded-full bg-background text-xs font-black text-foreground">
                        G
                      </span>
            ...
            <div className="mx-auto max-w-5xl px-4 text-center sm:px-6 lg:px-8">
              ...
              <div className="flex flex-col justify-center gap-4 sm:flex-row">
                <button
                  type="button"
                  onClick={() => {
                    void addQuote('Origin', 'Destination', 'Weight', 'Standard')
                    setQuotesOpen(true)
                  }}
