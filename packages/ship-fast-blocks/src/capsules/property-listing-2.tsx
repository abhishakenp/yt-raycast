...
export const PropertyListingKimiPage2 = defineCapsule({
  ...
  lakebed: {
    ...
    queries: {
      savedProperties: ({ db }) => db.savedProperties.orderBy('createdAt').all(),
      tourRequests: ({ db }) => db.tourRequests.orderBy('createdAt').all(),
    },
    mutations: {
      saveProperty: ({ db }, propertyTitle: string, propertyAddress: string, propertyPrice: string) => {
        const existing = db.savedProperties.where('propertyTitle', propertyTitle).all()[0]
        if (existing) {
          db.savedProperties.delete(existing.id)
          return false
        }
        db.savedProperties.insert({ propertyTitle, propertyAddress, propertyPrice })
        return true
      },
      submitTourRequest: ({ db }, email: string, propertyTitle: string, preferredDate: string) => {
        db.tourRequests.insert({ email, propertyTitle, preferredDate, status: 'pending' })
  ...
  component: ({ props, lakebed }) => {
    ...
    const logos = {
      label:
        props.logos?.label ??
        "Recognized by the world's leading publications",
      items: props.logos?.items?.length
        ? props.logos.items
        : ["Forbes", "Wall Street Journal", "Architectural Digest", "Dwell", "Dezeen"],
    }
    ...
    const gallery = {
      ...
      heading:
        props.gallery?.heading ?? "Walk the rooms before you arrive.",
    ...
    const featureIcons = [
      <svg key="f1" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001 1h12a1 1 0 001-1v-5H9m-3 0v5a1 1 0 001 1h12" />
      </svg>,
      <svg key="f2" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>,
      <svg key="f3" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
      </svg>,
      <svg key="f4" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
      </svg>,
      <svg key="f5" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M15 10l4.553-4.553A1 1 0 0121 6v12a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
      </svg>,
      <svg key="f6" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    ...
    const socialIcons: Record<string, ReactNode> = {
      ...
      LinkedIn: (
        <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25v10.5a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 18.75V8.25m18 0A2.25 2.25 0 0018.75 6H5.25A2.25 2.25 0 003 8.25m18 0v2.25a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 10.5V8.25" />
        </svg>
      ),
    }
    ...
    const isPropertySaved = savedProperties?.some((p) => p.propertyTitle === hero.title) ?? false
    ...
    return (
      <div className={cn("min-h-svh bg-background font-sans text-foreground antialiased", props.className)}>
        {/* Navbar */}
                          ...
                          >
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="mt-4 flex items-center justify-between">
                                <button
                                  type="button"
                                  onClick={() => void removeSavedProperty(property.id)}
        ...
        <main>
          ...
          <section className="relative isolate overflow-hidden bg-foreground pt-32 pb-20 sm:pt-40 sm:pb-24 lg:pt-48 lg:pb-32">
            <div className="absolute inset-0 -z-10">
              <Image alt={hero.imageAlt} w={2400} h={1600} className="h-full w-full object-cover opacity-40" />
              <div className="absolute inset-0 bg-gradient-to-t from-foreground via-foreground/60 to-transparent" />
            </div>
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
              <div className="mx-auto max-w-3xl text-center">
                <p className="text-sm font-bold uppercase tracking-widest text-primary mb-6">{hero.eyebrow}</p>
                <h1 className="text-5xl font-extrabold tracking-tight text-background sm:text-7xl lg:text-8xl">{hero.title}</h1>
                <p className="mt-6 text-lg leading-8 text-background/70 sm:text-xl">{hero.description}</p>
                <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
                  ...
                  <button
                    type="button"
                    onClick={() => void saveProperty(hero.title, hero.eyebrow, hero.stats[0]?.value || '')}
                    aria-pressed={isPropertySaved}
                    aria-label={isPropertySaved ? 'Remove from saved properties' : 'Save to properties'}
                    className={cn(
                ...
                <dl className="mt-16 grid grid-cols-2 gap-8 border-t border-background/10 pt-8 sm:grid-cols-4 sm:gap-x-12">
                  {hero.stats.map((s) => (
                    <div key={s.label}>
                      <dt className="text-sm font-medium text-background/60">{s.label}</dt>
                      <dd className="mt-2 text-3xl font-extrabold tracking-tight text-background">
                        {s.value}
                        {s.unit && <span className="text-xl font-semibold text-background/60"> {s.unit}</span>}
                      </dd>
                    </div>
                  ))}
                </dl>
          ...
          <section className="bg-muted py-14 border-b border-border">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
              <p className="text-center text-sm font-semibold uppercase tracking-widest text-muted-foreground mb-10">{logos.label}</p>
              <div className="grid grid-cols-2 gap-x-8 gap-y-10 sm:grid-cols-3 lg:grid-cols-5 items-center justify-items-center opacity-70">
                {logos.items.map((item) => (
                  <span key={item} className="text-xl font-serif font-bold text-foreground tracking-tight">{item}</span>
                ))}
              </div>
            </div>
          </section>
          ...
          <section className="py-24 sm:py-32 bg-background">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
              <div className="mx-auto max-w-2xl text-center">
                <h2 className="text-base font-bold uppercase tracking-widest text-primary mb-3">{features.eyebrow}</h2>
                <p className="text-4xl font-extrabold tracking-tight text-foreground sm:text-5xl">{features.heading}</p>
                <p className="mt-6 text-lg leading-8 text-muted-foreground">{features.description}</p>
              </div>
              <div className="mx-auto mt-16 grid max-w-7xl grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
                {features.items.map((item, i) => (
                  <div key={item.title} className="rounded-3xl border border-border bg-card p-8 shadow-sm hover:shadow-md transition-shadow">
          ...
          <section className="bg-foreground py-24 sm:py-32">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
              <div className="mx-auto max-w-2xl text-center">
                <h2 className="text-base font-bold uppercase tracking-widest text-primary mb-3">{steps.eyebrow}</h2>
                <p className="text-4xl font-extrabold tracking-tight text-background sm:text-5xl">{steps.heading}</p>
                <p className="mt-6 text-lg leading-8 text-background/70">{steps.description}</p>
              </div>
              <div className="mx-auto mt-16 grid max-w-7xl grid-cols-1 gap-10 sm:grid-cols-3">
                {steps.items.map((item) => (
                  <div key={item.number} className="relative rounded-3xl bg-card p-8 border border-border">
                    <span className="absolute -top-6 left-8 inline-flex h-12 w-12 items-center justify-center rounded-full bg-primary text-sm font-bold text-primary-foreground">{item.number}</span>
          ...
          <section className="py-24 sm:py-32 bg-background">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
              <div className="mx-auto max-w-2xl text-center">
                <h2 className="text-base font-bold uppercase tracking-widest text-primary mb-3">{gallery.eyebrow}</h2>
                <p className="text-4xl font-extrabold tracking-tight text-foreground sm:text-5xl">{gallery.heading}</p>
              </div>
              <div className="mt-16 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {gallery.items.map((item) => (
                  <figure key={item.caption} className={cn("group relative overflow-hidden rounded-3xl", item.wide && "sm:col-span-2 lg:col-span-2")}>
                    <Image alt={item.imageAlt} w={item.wide ? 1600 : 1200} h={800} className="h-72 w-full object-cover transition-transform duration-500 group-hover:scale-105" />
                    <figcaption className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-foreground/70 to-transparent px-6 pb-5 pt-10">
                      <span className="text-sm font-bold text-background">{item.caption}</span>
                    </figcaption>
                  </figure>
                ))}
              </div>
          ...
          <section className="bg-muted py-24 sm:py-32">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
              <div className="mx-auto max-w-2xl text-center">
                <h2 className="text-base font-bold uppercase tracking-widest text-primary mb-3">{pricing.eyebrow}</h2>
                <p className="text-4xl font-extrabold tracking-tight text-foreground sm:text-5xl">{pricing.heading}</p>
              </div>
              <div className="mx-auto mt-16 grid max-w-5xl grid-cols-1 gap-8 lg:grid-cols-2">
                {/* Sale */}
                <div className="rounded-3xl bg-card p-10 shadow-sm ring-1 ring-border flex flex-col justify-between">
                  <div>
                    <h3 className="text-xl font-bold text-foreground">{pricing.sale.title}</h3>
                    <p className="mt-2 text-sm text-muted-foreground">{pricing.sale.description}</p>
                    <p className="mt-6 text-5xl font-extrabold tracking-tight text-foreground">{pricing.sale.price}</p>
                    <ul className="mt-8 space-y-4 text-base text-muted-foreground">
                      {pricing.sale.features.map((f) => (
                        <li key={f} className="flex items-start gap-3">
                          <span className="mt-1 inline-flex h-5 w-5 items-center justify-center rounded-full bg-primary/20 text-primary text-xs font-bold">✓</span>
                          {f}
                        </li>
                      ))}
                    </ul>
                  ...
                  <button
                    type="button"
                    onClick={() => void saveProperty(hero.title, hero.eyebrow, pricing.sale.price)}
                    aria-pressed={isPropertySaved}
                    aria-label={isPropertySaved ? 'Remove from saved properties' : 'Save to properties'}
                    className={cn(
                ...
                <div className="rounded-3xl bg-foreground p-10 shadow-lg flex flex-col justify-between">
                  <div>
                    <h3 className="text-xl font-bold text-background">{pricing.lease.title}</h3>
                    <p className="mt-2 text-sm text-background/70">{pricing.lease.description}</p>
                    <p className="mt-6 text-5xl font-extrabold tracking-tight text-background">
                      {pricing.lease.price}<span className="text-xl font-semibold text-background/70"> {pricing.lease.unit}</span>
                    </p>
                    <ul className="mt-8 space-y-4 text-base text-background/70">
                      {pricing.lease.features.map((f) => (
                        <li key={f} className="flex items-start gap-3">
                          <span className="mt-1 inline-flex h-5 w-5 items-center justify-center rounded-full bg-primary text-primary-foreground text-xs font-bold">✓</span>
                          {f}
                        </li>
                      ))}
                    </ul>
                  ...
                  <button
                    type="button"
                    onClick={() => void saveProperty(hero.title, hero.eyebrow, pricing.lease.price)}
                    aria-pressed={isPropertySaved}
                    aria-label={isPropertySaved ? 'Remove from saved properties' : 'Save to properties'}
                    className={cn(
                      'mt-3 block w-full rounded-full px-6 py-3 text-center text-sm font-semibold transition-colors border',
              ...
              <div className="grid grid-cols-1 gap-y-12 sm:grid-cols-2 lg:grid-cols-4">
                {stats.items.map((s) => (
                  <div key={s.label} className="text-center">
                    <p className="text-5xl font-extrabold tracking-tight text-primary">{s.value}</p>
                    <p className="mt-2 text-base font-semibold text-foreground">{s.label}</p>
                    <p className="mt-1 text-sm text-muted-foreground">{s.sublabel}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <section className="py-24 sm:py-32 bg-muted">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
              <div className="mx-auto max-w-2xl text-center">
                <h2 className="text-base font-bold uppercase tracking-widest text-primary mb-3">{testimonials.eyebrow}</h2>
                <p className="text-4xl font-extrabold tracking-tight text-foreground sm:text-5xl">{testimonials.heading}</p>
              </div>
              <div className="mx-auto mt-16 grid max-w-7xl grid-cols-1 gap-8 lg:grid-cols-3">
                {testimonials.items.map((t) => (
                  <div key={t.name} className="rounded-3xl bg-card p-8 shadow-sm ring-1 ring-border">
                    <div className="flex items-center gap-4">
                      <Image alt={t.avatarAlt} w={120} h={120} className="h-12 w-12 rounded-full object-cover" />
                      <div>
                        <p className="text-sm font-bold text-foreground">{t.name}</p>
                        <p className="text-xs text-muted-foreground">{t.role}</p>
                      </div>
                    </div>
                    <p className="mt-6 text-base leading-7 text-muted-foreground">“{t.quote}”</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <section className="py-24 sm:py-32 bg-background">
            <div className="mx-auto max-w-4xl px-6 lg:px-8">
              <div className="text-center">
                <h2 className="text-base font-bold uppercase tracking-widest text-primary mb-3">{faq.eyebrow}</h2>
                <p className="text-4xl font-extrabold tracking-tight text-foreground sm:text-5xl">{faq.heading}</p>
                <p className="mt-6 text-lg leading-8 text-muted-foreground">{faq.description}</p>
              </div>
              <dl className="mt-16 space-y-6">
                {faq.items.map((item) => (
                  <div key={item.question} className="rounded-2xl border border-border p-6">
                    <dt className="text-base font-bold text-foreground">{item.question}</dt>
                    <dd className="mt-3 text-base leading-7 text-muted-foreground">{item.answer}</dd>
                  </div>
                ))}
              </dl>
            </div>
          ...
          <section className="relative isolate overflow-hidden bg-foreground py-24 sm:py-32">
            <div className="absolute inset-0 -z-10">
              <Image alt={tour.imageAlt} w={2400} h={1600} className="h-full w-full object-cover opacity-30" />
              <div className="absolute inset-0 bg-gradient-to-r from-foreground via-foreground/80 to-foreground/40" />
            </div>
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
              <div className="mx-auto max-w-2xl text-center">
                <h2 className="text-base font-bold uppercase tracking-widest text-primary mb-4">{tour.eyebrow}</h2>
                <p className="text-4xl font-extrabold tracking-tight text-background sm:text-5xl">{tour.heading}</p>
                <p className="mt-6 text-lg leading-8 text-background/70">{tour.description}</p>
                <form
                  className="mt-10 space-y-4 sm:space-y-0 sm:flex sm:items-center sm:justify-center sm:gap-4"
                  onSubmit={(e) => {
                    e.preventDefault()
                    const email = (e.currentTarget.elements.namedItem('tour-email') as HTMLInputElement).value
                    void submitTourRequest(email, hero.title, new Date().toISOString())
                    go(tour.submitLabel)
                  }}
                >
                ...
                <p className="mt-4 text-sm text-background/60">
                  Or call directly: <button type="button" onClick={() => go(tour.phone)} className="font-semibold text-background hover:text-primary transition-colors">{tour.phone}</button>
                </p>
              </div>
            </div>
          </section>
        ...
        <footer className="bg-foreground border-t border-border">
          <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
            <div className="grid grid-cols-1 gap-12 sm:grid-cols-2 lg:grid-cols-4">
              <div>
                <button type="button" onClick={() => go(nav[0])} className="flex items-center gap-2">
                  <span className="text-lg font-bold tracking-tight text-background">{brand}</span>
                  <span className="inline-flex h-2 w-2 rounded-full bg-primary" />
                </button>
                <p className="mt-4 text-sm leading-6 text-background/60">{footer.description}</p>
              </div>
              <div>
                <h4 className="text-sm font-bold uppercase tracking-widest text-background mb-4">Property</h4>
                <ul className="space-y-3 text-sm text-background/60">
                  {footer.propertyLinks.map((link) => (
                    <li key={link}><button type="button" onClick={() => go(link)} className="hover:text-primary transition-colors">{link}</button></li>
                  ))}
                </ul>
              </div>
              <div>
                <h4 className="text-sm font-bold uppercase tracking-widest text-background mb-4">Company</h4>
                <ul className="space-y-3 text-sm text-background/60">
              ...
              <div>
                <h4 className="text-sm font-bold uppercase tracking-widest text-background mb-4">{footer.contactLabel}</h4>
                <ul className="space-y-3 text-sm text-background/60">
                  {footer.contactLines.map((line) => (
                    <li key={line}><button type="button" onClick={() => go(line)} className="hover:text-primary transition-colors">{line}</button></li>
                  ))}
                </ul>
              </div>
            </div>
