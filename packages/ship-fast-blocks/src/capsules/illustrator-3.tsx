...
export const IllustratorKimiPage3 = defineCapsule({
  ...
  lakebed: {
    ...
    queries: {
      commissions: ({ db }) => db.commissions.orderBy('createdAt').all(),
      favoriteArtworkTitles: ({ db }) =>
        new Set(db.favorites.all().map((favorite) => favorite.artworkTitle)),
    ...
    mutations: {
      createCommission: ({ db }, title: string, description: string, budget: string) => {
        ...
        return db.commissions.all()
      },
      updateCommissionStatus: ({ db }, id: string, status: string) => {
        const commission = db.commissions.get(id)
        if (commission) {
          db.commissions.update(id, { status })
        }
        return db.commissions.all()
      },
      toggleFavorite: ({ db }, artworkTitle: string) => {
        ...
        db.favorites.insert({ artworkTitle })
        return true
      },
              ...
              <Sheet open={commissionsOpen} onOpenChange={setCommissionsOpen}>
                ...
                >
                  <SheetHeader className="border-b border-border p-6">
                    <SheetTitle className="text-xl">Commissions</SheetTitle>
                    <SheetDescription>
                      {commissions && commissions.length > 0
                        ? `${commissions.length} commission${commissions.length === 1 ? '' : 's'} in progress.`
                        : 'Start a new commission project.'}
                    </SheetDescription>
                  </SheetHeader>
                    ...
                    {commissions && commissions.length > 0 ? (
                      ...
                      <div className="flex min-h-64 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-muted/40 px-6 text-center">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                          Start your first commission project below.
                        </p>
                      </div>
                    )}
                  </div>
