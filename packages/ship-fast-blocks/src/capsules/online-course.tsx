...
export const OnlineCourseKimiPage = defineCapsule({
  ...
  lakebed: {
    ...
    queries: {
      ...
      savedCourseTitles: ({ db }) =>
        new Set(db.savedCourses.all().map((saved) => saved.courseTitle)),
    },
    mutations: {
      ...
      toggleSaveCourse: ({ db }, courseTitle: string) => {
        const existingSaved = db.savedCourses
          .where('courseTitle', courseTitle)
          .all()[0]

        if (existingSaved) {
          db.savedCourses.delete(existingSaved.id)
          return false
        }

        db.savedCourses.insert({ courseTitle })
        return true
      },
    },
  },
    ...
    return (
      ...
      >
                        ...
                        {safeEnrolledCourses.map((item) => (
                          ...
                          >
                            <div className="aspect-square overflow-hidden rounded-lg bg-muted">
                              <Image
                                alt={`${item.course.category} course thumbnail: ${item.course.title}`}
                                w={180}
                                h={180}
        ...
        <CommandDialog
          ...
          description="Search the courses available on this platform."
          className="max-w-xl"
        >
          <CommandInput placeholder={`Search ${brand} courses...`} />
          <CommandList className="max-h-[420px]">
            ...
            <CommandGroup heading="Courses">
              {displayCourses.map((course) => (
                <CommandItem
                  key={course.title}
                  value={`${course.category} ${course.title} ${course.instructor}`}
                  onSelect={() => {
                    setSearchOpen(false)
                    go(course.title)
                  }}
        ...
        <main>
          ...
          <section className="bg-muted/50 py-20 sm:py-28">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                {displayCourses.map((course) => {
                  const isSaved = savedCourseTitles?.has(course.title) ?? false

                  return (
                    <article key={course.title} className="group">
                      <div className="relative mb-4 aspect-video overflow-hidden rounded-2xl bg-card">
                        <Image
                          alt={`${course.category} course thumbnail: ${course.title}`}
                          w={800}
                          h={450}
                        ...
                        <button
                          type="button"
                          onClick={() => void toggleSaveCourse(course.title)}
                          aria-pressed={isSaved}
                          aria-label={
                            isSaved
                              ? `Remove ${course.title} from saved courses`
                              : `Save ${course.title} for later`
                          }
                          className={cn(
