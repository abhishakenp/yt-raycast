...
export const AiProductKimiPage2 = defineCapsule({
  ...
  lakebed: {
    ...
    mutations: {
      saveDraft: ({ db }, title: string, body: string) => {
        db.drafts.insert({
          title,
          body,
                      ...
                      {safeDrafts.length ? (
                        ...
                        <div className="flex min-h-64 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-muted/40 px-6 text-center">
                          ...
                          <p className="mt-2 text-sm text-muted-foreground">
                            Generate content to save your first draft.
                          </p>
                        </div>
                      )}
                    </div>
                          ...
                          <div className="flex flex-wrap gap-2 pt-2">
                            {draftActions.map((action, i) => (
                              <button
                                ...
                                type="button"
                                onClick={() => {
                                  if (i === 0 && action === 'Insert') {
                                    void saveDraft(draftTitle, draftBody)
                                    setDraftsOpen(true)
                                  } else {
                                    go(action)
                                  }
              ...
              <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                {featureItems.map((item, i) => {
                  ...
                  return (
                    <article
                      key={item.title}
                      className="relative rounded-2xl border border-border bg-muted/50 p-8 transition-colors hover:border-primary/40"
                    >
                      <button
                        type="button"
