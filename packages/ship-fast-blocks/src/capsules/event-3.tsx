...
export const EventKimiPage3 = defineCapsule({
  ...
  lakebed: {
    ...
    queries: {
      ...
      mySchedule: ({ db }) =>
        db.scheduleItems.all().flatMap((item) => {
          const session = db.sessions.get(item.sessionId)
          return session ? [{ ...item, session }] : []
        }),
    },
    mutations: {
      addToSchedule: ({ db }, sessionName: string) => {
        const session = db.sessions.where('name', sessionName).all()[0]
        if (!session) return db.scheduleItems.all()
      ...
      clearSchedule: ({ db }) => {
        for (const item of db.scheduleItems.all()) {
          db.scheduleItems.delete(item.id)
        }
  ...
  component: ({ props, lakebed }) => {
    ...
    const mySchedule = lakebed.useQuery('mySchedule')
    ...
    const clearSchedule = lakebed.useMutation('clearSchedule')
              ...
              <Sheet open={scheduleOpen} onOpenChange={setScheduleOpen}>
                ...
                >
                  <SheetHeader className="border-b border-border p-6">
                    ...
                    <SheetDescription>
                      {scheduleCount > 0
                        ? `${scheduleCount} session${scheduleCount === 1 ? '' : 's'} in your schedule.`
                        : 'Your schedule is empty.'}
                    </SheetDescription>
                  </SheetHeader>
                  <div className="flex-1 overflow-y-auto px-6 py-5">
                    {safeScheduleItems.length ? (
                      <div className="space-y-4">
                        {safeScheduleItems.map((item) => (
                          <div
                            key={item.id}
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="mt-3">
                                <button
                                  type="button"
                                  onClick={() =>
                                    void removeFromSchedule(item.sessionId)
                                  }
                      ...
                      <div className="flex min-h-64 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-muted/40 px-6 text-center">
                        <p className="text-base font-semibold text-foreground">
                          No sessions in schedule
                        </p>
                        <p className="mt-2 text-sm text-muted-foreground">
                          Add sessions from the sections below to build your
                          personal schedule.
                        </p>
                      </div>
                    )}
                  ...
                  <SheetFooter className="border-t border-border p-6">
                    <Button
                      ...
                      disabled={!safeScheduleItems.length}
                    >
                      Clear Schedule
                    </Button>
                    <SheetClose asChild>
