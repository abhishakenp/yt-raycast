...
export const InteriorDesignKimiPage2 = defineCapsule({
  ...
  lakebed: {
    ...
    mutations: {
      ...
      toggleSaved: ({ db }, projectTitle: string) => {
        const existing = db.savedProjects.where('projectTitle', projectTitle).all()[0]
        if (existing) {
  ...
  component: ({ props, lakebed }) => {
    ...
    const nav = props.nav?.length ? props.nav : ["Atelier Verve", "Work", "Services", "Process", "Pricing", "About"]
    const hero = {
      eyebrow: "Interior Design / Variant 2",
      title: "Spaces that inspire living.",
      description: "Atelier Verve Interior Design Studio Atelier Verve Work Services Process Pricing About FAQ Book a Call Award-Winning Interior Design Spaces that inspire living. We design bold,...",
    ...
    const sections = props.sections?.length ? props.sections : [
      {
        ...
        items: ["Kind words from people we have worked with.", "Investment options for every scope.", "Questions we hear often."],
      },
      {
        eyebrow: "Experience",
        ...
        items: ["Ready to transform your space?", "Residential Design", "Hospitality & F&B"],
      },
      {
        eyebrow: "Proof",
      ...
      },
      {
        ...
        items: ["Project Management", "Discovery & Brief", "Concept & Design"],
      },
    ]
    const gallery = props.gallery?.length ? props.gallery : [
      { title: "How we bring a space to life.", alt: "professional headshot of a smiling woman with dark hair", caption: "Interior Design generated page detail" },
      { title: "Projects that defy the ordinary.", alt: "professional headshot of a man with curly hair and glasses", caption: "Interior Design generated page detail" },
      { title: "Kind words from people we have worked with.", alt: "professional headshot of a woman with blonde hair smiling", caption: "Interior Design generated page detail" },
    ]
    ...
    const handleSignIn = () => { if (!auth.isLoading) void lakebed.signInWithGoogle() }
              ...
              {isSignedIn ? (
                <Popover>
                  ...
                  <PopoverContent align="end" sideOffset={10} className="w-72 overflow-hidden rounded-xl border-border bg-background p-0 shadow-xl">
                    ...
                    <div className="p-2">
                      <button type="button" onClick={() => go('Account')} className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm font-medium text-foreground transition-colors hover:bg-muted">
                        Account <ArrowRight />
                      </button>
                    </div>
                    <div className="border-t border-border p-2">
                      <button type="button" onClick={handleSignOut} className="flex w-full items-center justify-center rounded-lg bg-foreground px-3 py-2 text-sm font-semibold text-background transition-colors hover:bg-foreground/90">
                        Sign out
                      </button>
                    </div>
                  </PopoverContent>
                ...
                <SheetContent side="right" className="w-full gap-0 p-0 sm:max-w-md">
                  ...
                  <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                    <div className="space-y-1">
                      <label htmlFor="inq-name" className="text-sm font-medium text-foreground">Name</label>
                      <input
                        id="inq-name"
                        type="text"
                        value={inquiryName}
                    ...
                    <div className="space-y-1">
                      <label htmlFor="inq-email" className="text-sm font-medium text-foreground">Email</label>
                      <input
                        id="inq-email"
                        type="email"
                        value={inquiryEmail}
                    ...
                    <div className="space-y-1">
                      <label htmlFor="inq-message" className="text-sm font-medium text-foreground">Message</label>
                      <textarea
                        id="inq-message"
                        rows={4}
                      ...
                      <div className="mt-4">
                        ...
                        <div className="space-y-2">
                          {safeInquiries.map((inq) => (
                            <div key={inq.id} className="flex items-start justify-between gap-3 rounded-lg border border-border bg-muted/40 px-3 py-2">
                              <div className="min-w-0">
                                ...
                                <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{inq.message}</p>
                              </div>
                              <button
                                type="button"
                  ...
                  <SheetFooter className="border-t border-border p-6 gap-2">
                    <Button
                      ...
                      onClick={() => {
                        void submitInquiry(inquiryName.trim(), inquiryEmail.trim(), inquiryMessage.trim())
                        setInquiryName('')
                        setInquiryEmail('')
                        setInquiryMessage('')
                      }}
