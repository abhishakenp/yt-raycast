...
export const PlumbingHvacKimiPage = defineCapsule({
  ...
  lakebed: {
    ...
    queries: {
      serviceRequests: ({ db }) =>
        db.serviceRequests.orderBy('createdAt').all(),
      favoriteServiceNames: ({ db }) =>
        new Set(db.favorites.all().map((favorite) => favorite.serviceName)),
    },
    mutations: {
      bookService: ({ db }, serviceName: string, description: string) => {
        db.serviceRequests.insert({
          serviceName,
          description,
          status: 'pending',
        })
      ...
      cancelRequest: ({ db }, requestId: string) => {
        db.serviceRequests.delete(requestId)
        return db.serviceRequests.all()
      },
  ...
  component: ({ props, lakebed }) => {
    ...
    const bookService = lakebed.useMutation('bookService')
    const cancelRequest = lakebed.useMutation('cancelRequest')
    ...
    const safeServiceRequests = serviceRequests ?? []
            ...
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {serviceItems.map((item, i) => {
                ...
                return (
                  <div
                    key={item.title}
                    className="relative rounded-2xl bg-muted p-8 transition-shadow hover:shadow-lg"
                  >
                    <button
                      type="button"
                      ...
                      className={cn(
                        'absolute right-6 top-6 grid size-10 place-items-center rounded-full shadow-md transition-all hover:scale-105',
                        isFavorite
                    ...
                    <Button
                      ...
                      className="w-full rounded-full"
                      onClick={() => {
                        void bookService(item.title, 'Service request')
                        setBookingOpen(true)
                      }}
                    >
                      Book Service
                ...
                >
                  ...
                  <div className="p-8">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        void bookService(plan.name, 'Pricing plan request')
                        setBookingOpen(true)
                      }}
                      className={cn(
          ...
          <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col justify-center gap-4 sm:flex-row">
              ...
              <button
                type="button"
                onClick={() => {
                  void bookService('Emergency Service', 'Emergency request')
                  setBookingOpen(true)
                }}
