...
export const LogisticsKimiPage3 = defineCapsule({
  ...
  lakebed: {
    ...
    mutations: {
      addShipment: ({ db }, trackingNumber: string, origin: string, destination: string, status: string, weight: string) => {
        db.shipments.insert({ trackingNumber, origin, destination, status, weight })
      ...
      addQuote: ({ db }, serviceType: string, origin: string, destination: string, weight: string, estimatedCost: string) => {
        db.quotes.insert({ serviceType, origin, destination, weight, estimatedCost })
  ...
  component: ({ props, lakebed }) => {
    ...
    const brand = props.brand ?? "Atlas Global Logistics Worldwide Shipping & Freight Solutions"
    const nav = props.nav?.length ? props.nav : ["Atlas Global", "Services", "Solutions", "Pricing", "Track", "Company"]
    ...
    const hero = {
      ...
      description: "Atlas Global Logistics Worldwide Shipping & Freight Solutions Atlas Global Services Solutions Pricing Track Company Client Login Get a Quote Now serving 195 countries with real-...",
      primaryCta: "Track",
      secondaryCta: "Subscribe",
      imageAlt: "Aerial view of a large container shipping port with cranes and stacked cargo containers",
      ...props.hero,
    }
      ...
      <div className={cn("min-h-screen bg-background text-foreground", props.className)}>
        <header className="sticky top-0 z-30 border-b border-border bg-background/90 backdrop-blur">
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4">
                    ...
                    {safeShipments.length ? (
                      <div className="space-y-4">
                        {safeShipments.map((shipment) => (
                          <div key={shipment.id} className="rounded-lg border border-border bg-card p-4">
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Tracking #{shipment.trackingNumber}</p>
                                <h3 className="text-sm font-semibold text-foreground">{shipment.origin} → {shipment.destination}</h3>
                                <p className="mt-1 text-sm text-muted-foreground">Weight: {shipment.weight}</p>
                              </div>
                              <span className={cn(
                                "inline-flex items-center rounded-full px-2 py-1 text-xs font-medium",
                                shipment.status === 'Delivered' ? "bg-green-500/10 text-green-700 dark:bg-green-500/20 dark:text-green-400" :
                                shipment.status === 'In Transit' ? "bg-blue-500/10 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400" :
                                shipment.status === 'Pending' ? "bg-yellow-500/10 text-yellow-700 dark:bg-yellow-500/20 dark:text-yellow-400" :
                      ...
                      <div className="flex min-h-64 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-muted/40 px-6 text-center">
                        <p className="text-base font-semibold text-foreground">No shipments tracked</p>
                        <p className="mt-2 text-sm text-muted-foreground">Add a tracking number to monitor your shipments.</p>
                      </div>
                    )}
                  </div>
                  <SheetFooter className="border-t border-border p-6">
                    <Button type="button" className="w-full rounded-full" onClick={() => go('New Shipment')}>
                      Add Shipment
                    </Button>
                    <SheetClose asChild>
            ...
            {mobileOpen && (
              <div id="mobile-menu" className="flex flex-col border-t border-border bg-background px-4 py-6 pb-8 md:hidden gap-4">
                {nav.map((label) => (
                  <button
                    key={label}
                    type="button"
                    onClick={() => { setMobileOpen(false); go(label) }}
              ...
              {safeQuotes.length ? (
                <div className="space-y-4">
                  {safeQuotes.map((quote) => (
                    <div key={quote.id} className="rounded-lg border border-border bg-card p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">{quote.serviceType}</p>
                          <h3 className="text-sm font-semibold text-foreground">{quote.origin} → {quote.destination}</h3>
                          <p className="mt-1 text-sm text-muted-foreground">Weight: {quote.weight}</p>
                          <p className="mt-1 text-sm font-bold text-foreground">Est. Cost: {quote.estimatedCost}</p>
                        </div>
                        <button
                          type="button"
                ...
                <div className="flex min-h-64 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-muted/40 px-6 text-center">
                  <p className="text-base font-semibold text-foreground">No quotes requested</p>
                  <p className="mt-2 text-sm text-muted-foreground">Request a quote to get started with your shipment.</p>
                </div>
              )}
            </div>
            <SheetFooter className="border-t border-border p-6">
              <Button type="button" className="w-full rounded-full" onClick={() => go('New Quote')}>
                Request Quote
              </Button>
              <SheetClose asChild>
                <Button type="button" variant="secondary" className="rounded-full">
