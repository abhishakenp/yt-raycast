...
export const ProductDetailKimiPage = defineCapsule({
  ...
  lakebed: {
    ...
    mutations: {
      addToCart: ({ db }, productName: string, productPrice: string, productImage: string, selectedSize: string, selectedColor: string, quantity: number) => {
        ...
        if (existingItem) {
          ...
          db.cartItems.insert({
            productId: `${productName}-${selectedSize}-${selectedColor}`,
            productName,
            productPrice,
            productImage,
            quantity,
  ...
  component: ({ props, lakebed }) => {
    ...
    const cartSubtotal = safeCartLines.reduce(
      (total, item) => total + priceAmount(item.productPrice) * item.quantity,
      0,
    )
    const shipping = cartSubtotal > 0 && cartSubtotal < 200 ? 12 : 0
    ...
    const colorLabel = p.colorLabel ?? selectedColor
    ...
    const addToCartLabel = p.addToCart ?? "Add to Cart"
                      ...
                      {safeCartLines.length ? (
                        ...
                        <div className="flex min-h-64 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-muted/40 px-6 text-center">
                          ...
                          <p className="mt-2 text-sm text-muted-foreground">
                            Add an item to start a cart for this session.
                          </p>
                        </div>
                      )}
                    </div>
              ...
              <div className="flex-1 space-y-6">
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      void addToCart(productTitle, productPrice, images[0], selectedSize, selectedColor, quantity)
                      setCartOpen(true)
                    }}
          ...
          <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:gap-6">
              {relatedItems.map((item) => {
                const isFavorite = favoriteProductNames?.has(item.title) ?? false
                return (
                  <div
                    key={item.title}
