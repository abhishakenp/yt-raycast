...
export const EventPlannerKimiPage2 = defineCapsule({
  ...
  lakebed: {
    ...
    mutations: {
      submitInquiry: ({ db }, name: string, email: string, eventType: string, date: string, message: string) => {
        db.inquiries.insert({
          name,
          email,
          eventType,
  ...
  component: ({ props, lakebed }) => {
    ...
    const nav = props.nav?.length ? props.nav : ["Services", "Process", "Gallery", "Pricing", "FAQ", "Start Planning"]
    const hero = {
      eyebrow: "Event Planner / Variant 2",
      title: "Events that leave a mark.",
      description: "Aura Event Design | Austin's Premier Event Planning Studio AURA . Services Process Gallery Pricing FAQ Start Planning Services Process Gallery Pricing FAQ Start Planning Rated 5...",
      primaryCta: "Send Inquiry",
      secondaryCta: "AURA .",
      imageAlt: "elegant wedding reception long table adorned with glowing candles and suspended Edison bulbs",
      ...props.hero,
    }
  ...
  },
  {
    ...
    const handleInquirySubmit = (e: React.FormEvent) => {
      ...
      setInquiryForm({ name: '', email: '', eventType: '', date: '', message: '' })
        ...
        <main>
          ...
          <section className="mx-auto max-w-7xl px-5 py-16">
            ...
            <div className="grid gap-5 md:grid-cols-3">
              {gallery.map((item) => {
                const isFavorite = favoriteGalleryTitles?.has(item.title) ?? false
                return (
