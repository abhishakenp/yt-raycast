...
export const MobileAppKimiPage3 = defineCapsule({
  ...
  component: ({ props, lakebed }) => {
    ...
    /* ── Steps defaults ── */
    const stepsHeading = props.steps?.heading ??
      "Start Building Better Habits In 3 Simple Steps"
    const stepsDesc =
    ...
    const ctaPlatforms =
      props.cta?.platforms ?? "Available on iOS 15+, Android 8+, Apple Watch, and Web"
    ...
    const footerColumns = props.footer?.columns?.length
      ...
      : [
          ...
          {
            title: "Support",
            links: ["Help Center", "Community", "API Docs", "Status", "Security"],
          },
        ]
    ...
    const ChevronDownIcon = () => (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-5 text-muted-foreground" aria-hidden="true">
    ...
    const featureIcons = [
      ...
      <svg key="0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-6" aria-hidden="true">
        <path d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
      </svg>,
      /* 1 — reminders (clock) */
      <svg key="1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-6" aria-hidden="true">
        <path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>,
      /* 2 — community (users) */
      <svg key="2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-6" aria-hidden="true">
        <path d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
      </svg>,
      /* 3 — achievements (star/sparkle) */
      <svg key="3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-6" aria-hidden="true">
        <path d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
      </svg>,
      /* 4 — habit stacking (heart) */
      <svg key="4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-6" aria-hidden="true">
        <path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
      </svg>,
      /* 5 — widget (phone) */
      <svg key="5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="size-6" aria-hidden="true">
    ...
    return (
      ...
      >
            ...
            <div className="flex h-16 items-center justify-between lg:h-20">
              ...
              <div className="flex items-center gap-4">
                {isSignedIn ? (
                  ...
                  <button
                    type="button"
                    onClick={() => !auth.isLoading && void lakebed.signInWithGoogle()}
                    ...
                    <div className="flex-1 overflow-y-auto px-6 py-5">
                      {submitted ? (
                        <div className="flex min-h-48 flex-col items-center justify-center gap-4 text-center">
                          ...
                          <p className="text-base font-semibold">Welcome aboard!</p>
                          <p className="text-sm text-muted-foreground">We'll notify you at <span className="font-medium text-foreground">{waitlistEmail}</span> when we launch your plan.</p>
                        </div>
                      ) : (
                        <form onSubmit={handleWaitlistSubmit} className="space-y-4">
                          <div>
                            <label htmlFor="waitlist-email" className="mb-1.5 block text-sm font-medium">Email address</label>
                            <input
                              id="waitlist-email"
                              type="email"
                              required
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              {(props.pricing?.tiers?.length ? props.pricing.tiers : pricingTiers).map((tier) => (
                                <button
                                  key={tier.name}
                                  type="button"
                                ...
                                >
                                  ...
                                  <span>{tier.price}{tier.period ? tier.period : ""}</span>
                                </button>
                              ))}
                            </div>
                          </div>
                      ...
                      {(waitlistEntries ?? []).length > 0 && (
                        <div className="mt-6 rounded-xl border border-border bg-muted/30 p-4">
                          <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Recent sign-ups</p>
                          <div className="space-y-2">
                            {(waitlistEntries ?? []).slice(-5).reverse().map((entry) => (
                              <div key={entry.id} className="flex items-center justify-between text-sm">
                                <span className="truncate text-foreground/80">{entry.email}</span>
                                <span className="ml-2 shrink-0 rounded-full bg-primary/10 px-2 py-0.5 text-xs text-primary">{entry.plan}</span>
                              </div>
                            ))}
                          </div>
                        </div>
        ...
        <main>
          {/* ── Hero ── */}
          <section className="relative overflow-hidden pb-20 pt-32 lg:pb-32 lg:pt-40" aria-labelledby="hero-heading">
            <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-gradient-to-b from-primary/10 via-transparent to-transparent" />
            <div aria-hidden="true" className="pointer-events-none absolute left-1/2 top-1/2 size-[800px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary/20 blur-3xl" />
              ...
              <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
                <div className="text-center lg:text-left">
                  ...
                  <div className="flex flex-col gap-4 sm:flex-row justify-center lg:justify-start">
                    <button
                      type="button"
                      onClick={() => { void trackDownload("ios"); setWaitlistOpen(true) }}
                    ...
                    <button
                      type="button"
                      onClick={() => { void trackDownload("android"); setWaitlistOpen(true) }}
                  ...
                  <div className="mt-8 flex items-center justify-center gap-4 text-sm text-muted-foreground lg:justify-start">
                    <div className="flex items-center gap-1">
                      <StarIcon />
                      <span className="font-medium text-foreground">{appStoreRating}</span>
                      <span>App Store</span>
                    </div>
                    <span className="text-border">•</span>
                    <div className="flex items-center gap-1">
                      <StarIcon />
                      <span className="font-medium text-foreground">{googlePlayRating}</span>
                ...
                <div className="relative flex justify-center lg:justify-end">
                  <div className="relative">
                    <div aria-hidden="true" className="absolute -inset-4 rounded-[3rem] bg-gradient-to-r from-primary/30 to-accent/30 blur-2xl" />
                    <Image
                    ...
                    <div className="absolute -bottom-6 -left-6 rounded-2xl border border-border bg-card p-4 shadow-xl">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xl font-bold text-card-foreground">{streakValue}</p>
                        </div>
                      </div>
                    </div>
          ...
          {/* ── Logos ── */}
          <section className="border-y border-border bg-muted/30 py-12" aria-label="Featured in">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex flex-wrap items-center justify-center gap-8 opacity-60 sm:gap-12 lg:gap-16">
                {logoItems.map((name) => (
                  <span key={name} className="text-lg font-bold text-muted-foreground sm:text-xl">
                    {name}
                  </span>
                ))}
              </div>
          ...
          {/* ── Steps ── */}
          <section className="bg-muted/30 py-20 lg:py-32" aria-labelledby="steps-heading">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              <div className="mx-auto mb-16 max-w-3xl text-center lg:mb-20">
                <h2
                  id="steps-heading"
          ...
          <section aria-label="App gallery" className="overflow-hidden py-20 lg:py-32">
            <div className="mx-auto mb-12 max-w-7xl px-4 sm:px-6 lg:px-8">
              <h2 className="text-center text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">
                {galleryHeading}
              </h2>
            </div>
            <div className="flex snap-x snap-mandatory gap-6 overflow-x-auto px-4 pb-4" style={{ scrollbarWidth: "none" }}>
          ...
          {/* ── Pricing ── */}
          <section className="bg-muted/30 py-20 lg:py-32" aria-labelledby="pricing-heading">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          {/* ── Testimonials ── */}
          <section className="bg-muted/30 py-20 lg:py-32" aria-labelledby="testimonials-heading">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              <div className="mx-auto mb-16 max-w-3xl text-center">
                <h2
            ...
            <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
              <div className="relative rounded-3xl border border-primary/30 bg-gradient-to-b from-primary/20 to-primary/5 p-8 lg:p-12">
                ...
                <div className="relative">
                  ...
                  <p className="text-sm text-muted-foreground">{ctaPlatforms}</p>
                </div>
              </div>
            </div>
          </section>
