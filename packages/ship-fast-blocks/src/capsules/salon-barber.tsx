...
export const SalonBarberKimiPage = defineCapsule({
  ...
  lakebed: {
    ...
    queries: {
      ...
      bookingLines: ({ db }) =>
        db.bookings.all().map((item) => ({
          ...item,
        })),
      favoriteServiceTitles: ({ db }) =>
    ...
    mutations: {
      ...
      removeFromBookings: ({ db }, id: string) => {
      ...
      toggleFavorite: ({ db }, serviceTitle: string) => {
        ...
        db.favorites.insert({ serviceTitle })
        return true
      },
    },
  },
  component: ({ props, lakebed }) => {
    ...
    const normalizedServiceItems = serviceItems.map((service) => ({
      title: service.title,
    ...
    const favoriteServiceTitles = lakebed.useQuery('favoriteServiceTitles')
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                {displayServices.map((item) => {
                  const isFavorite =
                    favoriteServiceTitles?.has(item.title) ?? false

                  return (
                    <div
                      key={item.title}
                      className="relative rounded-xl border border-border bg-card p-8 shadow-sm transition-shadow hover:shadow-md"
                    >
                      <button
                        type="button"
                ...
                {faqItems.map((item) => (
                  ...
                  >
                    <summary className="flex cursor-pointer items-center justify-between p-6">
                      ...
                      <span className="ml-6 flex size-5 flex-shrink-0 items-center justify-center">
                        <ChevronDown />
                      </span>
                    </summary>
