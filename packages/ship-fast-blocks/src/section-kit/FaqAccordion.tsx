...
const FaqAccordion = React.forwardRef<
 ...
 VariantProps<typeof faqAccordionVariants> & { asChild?: boolean }>(({ className, variant, asChild = false, ...props }, ref) => {
...
const faqItemVariants = cva('group', {
 ...
 muted: ' bg-muted/50',
 'bordered-lg': ' border border-border bg-card',
 minimal: ' bg-background',
 divided: 'py-5',
 'open-raised':
 ' border border-border bg-muted/40 transition-all open:bg-card open:',
 'overflow-bordered':
...
const FaqItem = React.forwardRef<
 ...
 VariantProps<typeof faqItemVariants> & { asChild?: boolean }>(({ className, variant, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'details'
 return (
 <Comp
 data-slot="faq-item"
 data-d-role="card"className={cn(faqItemVariants({ variant }), className)}
 ref={ref}
 {...props}
 />
 )
...
const FaqQuestion = React.forwardRef<
 HTMLElement,
 React.ComponentProps<'summary'> & { asChild?: boolean }>(({ className, asChild = false, ...props }, ref) => {
...
const FaqQuestionIcon = React.forwardRef<
 ...
 VariantProps<typeof faqQuestionIconVariants> & { asChild?: boolean }>(({ className, variant, asChild = false, children, ...props }, ref) => {
 ...
 data-d-role="heading"className={cn(faqQuestionIconVariants({ variant }), className)}
 ref={ref}
 {...props}>
 {children ?? icon}
 </Comp>
