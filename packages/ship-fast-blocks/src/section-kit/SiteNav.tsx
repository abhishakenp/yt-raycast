...
>(({ className, href, asChild = false, ...props }, ref) => {
  ...
  return <Comp {...(brandProps as Record<string, unknown>)} />
})
...
>(({ className, href, asChild = false, onClick, ...props }, ref) => {
  ...
  const Comp = (asChild
    ...
    : resolvedHref
      ? RouterLink
      : 'a') as unknown as React.ComponentType<Record<string, unknown>>
  const onNavClick = useSectionKitNavClick(navHref)
  ...
  return <Comp {...(routeLinkProps as Record<string, unknown>)} />
})
...
>(({ className, href, variant, asChild = false, type, ...props }, ref) => {
  ...
  return <Comp {...(ctaProps as Record<string, unknown>)} />
})
...
function LegacySiteNav(props: SiteNavProps) {
  ...
  const borderClass =
    intent.shadow === 'shadow-[8px_8px_0_0]' ||
    intent.shadow === 'shadow-[4px_4px_0_0]'
  ...
  const ctaButton =
    cta && ctaIsAuth ? (
      ...
      <RouterLink
        href={ctaHref ?? '#'}
        data-d-role="btn"
        className={cn(kitActionClasses(cta.variant), 'hidden sm:inline-flex')}
      >
        {cta.label}
      </RouterLink>
    ) : null
