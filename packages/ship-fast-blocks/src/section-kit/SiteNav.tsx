...
import { useDesignIntent } from '#/primitives/design-context.tsx'
...
const navbarCtaVariants = cva(
 ...
 dark: ' bg-foreground px-4 py-2 text-background hover:bg-foreground/90',
 'dark-pill':
...
const NavbarBrand = React.forwardRef<
 HTMLElement,
 React.ComponentProps<'a'> & { asChild?: boolean }>(({ className, href, asChild = false, ...props }, ref) => {
 ...
 data-d-role="nav"href={resolvedHref}
 onClick={onNavClick}
...
const NavbarNav = React.forwardRef<
 HTMLDivElement,
 React.ComponentProps<'div'> & { asChild?: boolean; breakpoint?: 'md' | 'lg' }>(({ className, asChild = false, breakpoint = 'md', ...props }, ref) => {
 const Comp = asChild ? Slot : 'div'
 const bp = breakpoint === 'lg' ? 'lg:flex' : 'md:flex'
 return (
 <Comp
 data-slot="navbar-nav"
 data-d-role="nav"className={cn('hidden items-center gap-8', bp, className)}
 ref={ref}
 {...props}
 />
 )
...
const NavbarNavLink = React.forwardRef<
 ...
 onClick: onNavClick,
 className: cn(
 'relative whitespace-nowrap px-2.5 py-1.5 text-sm font-medium text-muted-foreground transition-all duration-200 ease-out hover:bg-muted hover:text-foreground',
...
const NavbarActions = React.forwardRef<
 ...
 data-d-role="nav"className={cn('flex items-center gap-4', className)}
 ref={ref}
 {...props}
 />
 )
...
const NavbarRouteLink = React.forwardRef<
 HTMLAnchorElement,
 React.ComponentProps<'a'> & { asChild?: boolean }>(({ className, href, asChild = false, onClick, ...props }, ref) => {
 const navHref = typeof href === 'string' ? href : undefined
 const resolvedHref = useSectionKitNavHref(navHref)
 ...
 data-d-role="link"href={resolvedHref}
 onClick={(event) => {
 onNavClick(event)
 onClick?.(event)
 }}
...
const NavbarCta = React.forwardRef<
 ...
 VariantProps<typeof navbarCtaVariants> & { asChild?: boolean }>(({ className, href, variant, asChild = false, type, ...props }, ref) => {
 const navHref = typeof href === 'string' ? href : undefined
 const resolvedHref = useSectionKitNavHref(navHref)
 ...
 data-d-role="btn"href={resolvedHref}
 onClick={onNavClick}
...
function LegacySiteNav(props: SiteNavProps) {
 ...
 // Design-aware border: hard offset shadow=border-2, sharp radius=border-b-2, others=border-b
 const borderClass =
 intent.shadow === 'shadow-[8px_8px_0_0]' || intent.shadow === 'shadow-[4px_4px_0_0]'
 ? 'border-b-2 border-foreground'
 : intent.radius === 'rounded-none'
 ...
 // Design-aware brand font: detect typography intent
 const isTechnical = intent.typography === 'technical'
 const isEditorial = intent.typography === 'editorial'
 const isDisplay = intent.typography === 'display'
...
const CompoundSiteNav = React.forwardRef<
 ...
 <Comp
 data-slot="site-nav"
 data-d-role="nav"className={headerClasses}
 ref={ref}
 {...props}>
 {children}
 </Comp>
