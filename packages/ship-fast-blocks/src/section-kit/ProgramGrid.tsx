...
const ProgramGrid = React.forwardRef<
 ...
 VariantProps<typeof programGridVariants> & { asChild?: boolean }>(({ className, cols, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'div'
 return (
 <Comp
 data-slot="program-grid"
 data-d-role="grid"className={cn(programGridVariants({ cols }), className)}
 ref={ref}
 {...props}
 />
 )
...
const ProgramIcon = React.forwardRef<
 HTMLDivElement,
 React.ComponentProps<'div'> &
 VariantProps<typeof programIconVariants> & { asChild?: boolean }>(({ className, shape, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'div'
 return (
 <Comp
