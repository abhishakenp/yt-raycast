...
const listingCardVariants = cva('group flex flex-col overflow-hidden', {
 variants: {
 variant: {
 'bordered-card': ' border border-border bg-card',
 'selectable-card': ' border bg-card',
 },
 },
 defaultVariants: {
...
const ListingCard = React.forwardRef<HTMLElement, ListingCardProps>(
 ...
 data-d-role="card"className={cn(listingCardVariants({ variant }), className)}
 {...props}
 />
 )
 },
...
const ListingCardMedia = React.forwardRef<
 ...
 data-d-role="card"className={cn('relative aspect-[4/3] overflow-hidden', className)}
 {...props}
 />
 )
})
...
const ListingCardBadge = React.forwardRef<
 HTMLSpanElement,
 ListingCardBadgeProps>(({ className, variant, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'span'
 return (
 <Comp
 ref={ref}
 data-slot="listing-card-badge"
 data-d-role="badge"className={cn(listingBadgeVariants({ variant }), className)}
 {...props}
 />
 )
})
