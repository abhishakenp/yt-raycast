...
const ArticleGrid = React.forwardRef<
 ...
 VariantProps<typeof articleGridVariants> & { asChild?: boolean }>(({ className, cols, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'div'
 return (
 <Comp
 data-slot="article-grid"
 data-d-role="grid"className={cn(articleGridVariants({ cols }), className)}
 ref={ref}
 {...props}
 />
 )
...
const ArticleContent = React.forwardRef<
 ...
 data-d-role="container"className={cn('flex flex-1 flex-col', className)}
 ref={ref}
 {...props}
 />
 )
