...
const SearchForm = React.forwardRef<
 ...
 VariantProps<typeof searchFormVariants> & { asChild?: boolean }>(({ className, layout, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'form'
 return (
 <Comp
 data-slot="search-form"
 data-d-role="form"className={cn(searchFormVariants({ layout }), className)}
 ref={ref}
 {...props}
 />
 )
