...
const TicketGrid = React.forwardRef<
 ...
 VariantProps<typeof ticketGridVariants> & { asChild?: boolean }>(({ className, cols, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'div'
 return (
 <Comp
 data-slot="ticket-grid"
 data-d-role="grid"className={cn(ticketGridVariants({ cols }), className)}
 ref={ref}
 {...props}
 />
 )
