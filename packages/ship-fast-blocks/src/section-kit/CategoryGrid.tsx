...
const CategoryGrid = React.forwardRef<
 ...
 VariantProps<typeof CategoryGridVariants> & { asChild?: boolean }>(({ className, cols, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'div'
 return (
 <Comp
 data-slot="category-grid"
 data-d-role="grid"className={cn(CategoryGridVariants({ cols }), className)}
 ref={ref}
 {...props}
 />
 )
...
const CategoryIcon = React.forwardRef<
 ...
 className={cn(
 'grid size-12 place-items-center text-lg',
 className,
 )}
 ref={ref}
 {...props}
