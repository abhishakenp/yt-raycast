...
const IndustryGrid = React.forwardRef<
 ...
 VariantProps<typeof IndustryGridVariants> & { asChild?: boolean }>(({ className, cols, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'div'
 return (
 <Comp
 data-slot="industry-grid"
 data-d-role="grid"className={cn(IndustryGridVariants({ cols }), className)}
 ref={ref}
 {...props}
 />
 )
