...
const ClassGrid = React.forwardRef<
 ...
 VariantProps<typeof classGridVariants> & { asChild?: boolean }>(({ className, cols, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'div'
 return (
 <Comp
 data-slot="class-grid"
 data-d-role="grid"className={cn(classGridVariants({ cols }), className)}
 ref={ref}
 {...props}
 />
 )
