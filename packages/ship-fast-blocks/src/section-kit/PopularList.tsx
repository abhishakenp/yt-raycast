...
const PopularList = React.forwardRef<
 ...
 VariantProps<typeof PopularListVariants> & { asChild?: boolean }>(({ className, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'ul'
 return (
 <Comp
 data-slot="popular-list"
 data-d-role="list"className={cn(PopularListVariants({}), className)}
 ref={ref}
 {...props}
 />
 )
...
const PopularCard = React.forwardRef<
 ...
 'flex items-center gap-4 border border-border bg-card p-4 text-left transition-colors hover:bg-muted/50',
 className,
 )}
 ref={ref}
 {...props}
