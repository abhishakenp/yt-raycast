...
const StepTimeline = React.forwardRef<
 ...
 VariantProps<typeof stepTimelineVariants> & { asChild?: boolean }>(({ className, variant, asChild = false, ...props }, ref) => {
...
const StepTimelineGrid = React.forwardRef<
 HTMLDivElement,
 React.ComponentProps<'div'> & { columns?: 2 | 3 | 4; asChild?: boolean }>(({ className, columns = 3, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'div'
 return (
 <Comp
...
const stepBadgeVariants = cva('flex items-center justify-center', {
 ...
 variant: {
 'filled-square':
 'size-12 bg-primary font-bold text-primary-foreground',
 'filled-circle':
 ...
 'gradient-square':
 'size-16 bg-gradient-to-br from-primary to-primary/80 text-2xl font-extrabold text-primary-foreground ring-4 ring-background',
 'outlined-circle':
...
const StepBadge = React.forwardRef<
 ...
 <Comp
 data-slot="step-badge"
 data-d-role="badge"className={cn(stepBadgeVariants({ variant }), className)}
 ref={ref}
 {...props}>
 {num}
 </Comp>
...
const StepConnector = React.forwardRef<
 HTMLDivElement,
 React.ComponentProps<'div'> &
 VariantProps<typeof stepConnectorVariants> & { asChild?: boolean }>(({ className, variant, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'div'
 return (
 <Comp
 aria-hidden="true"
...
const StepItem = React.forwardRef<
 ...
 <Comp
 data-slot="step-item"
 data-d-role="card"className={cn('relative', className)}
 ref={ref}
 {...props}
 />
 )
...
const StepContent = React.forwardRef<
 ...
 data-d-role="container"className={cn('mt-4 flex flex-col gap-3', className)}
 ref={ref}
 {...props}
 />
 )
