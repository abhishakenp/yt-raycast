...
/**
 * BentoTileCaption — overlay caption for a bento tile. The `reveal` CVA variant
 * controls visibility: `always` (visible by default) or `hover` (fades in on
 * group-hover). Gradient direction, opacity, padding, and text styling compose
 * via className — these vary per capsule and are single-utility overrides.
 */
const bentoCaptionVariants = cva('absolute', {
  variants: {
    reveal: {
      always: '',
      hover:
...
export interface BentoTileCaptionProps
  extends
    React.HTMLAttributes<HTMLDivElement>,
    VariantProps<typeof bentoCaptionVariants> {
  asChild?: boolean
}

const BentoTileCaption = React.forwardRef<
  HTMLDivElement,
  BentoTileCaptionProps
>(({ className, reveal, asChild = false, ...props }, ref) => {
  const Comp = asChild ? Slot : 'div'
  return (
    <Comp
      ref={ref}
      data-slot="bento-tile-caption"
      className={cn(bentoCaptionVariants({ reveal }), className)}
      {...props}
    />
  )
})
BentoTileCaption.displayName = 'BentoTileCaption'
