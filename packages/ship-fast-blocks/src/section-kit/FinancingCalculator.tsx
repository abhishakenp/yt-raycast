...
const FinancingCalculatorVariants = cva(' border border-border', {
 variants: {
 variant: {
...
const FinancingCalculator = React.forwardRef<
 ...
 VariantProps<typeof FinancingCalculatorVariants> & { asChild?: boolean }>(({ className, variant, asChild = false, ...props }, ref) => {
...
const FinancingDisplay = React.forwardRef<
 ...
 data-d-role="display"className={cn(' bg-primary/10 p-6 text-center', className)}
 ref={ref}
 {...props}
 />
 )
