...
const pricingTierVariants = cva(
 'relative flex h-full min-w-0 flex-col gap-6 border bg-card p-5 sm:p-6 lg:p-7',
 {
 variants: {
 variant: {
 default: 'border-border',
 highlighted:
 'border-primary bg-primary/[0.03] shadow-primary/10 ring-1 ring-primary/15',
 },
 },
...
const PricingTier = React.forwardRef<
 ...
 VariantProps<typeof pricingTierVariants> & { asChild?: boolean }>(({ className, variant, asChild = false, ...props }, ref) => {
 const Comp = asChild ? Slot : 'div'
 return (
 <Comp
 ref={ref}
 data-slot="pricing-tier"
 data-d-role="card"className={cn(pricingTierVariants({ variant }), className)}
 {...props}
 />
 )
})
...
const PricingTierHeader = React.forwardRef<
 ...
 data-d-role="card"className={cn('flex min-w-0 flex-col gap-2', className)}
 {...props}
 />
 )
})
...
const PricingTierTagline = React.forwardRef<
 ...
 data-d-role="card"className={cn('text-sm leading-6 text-muted-foreground', className)}
 {...props}
 />
 )
})
...
const PricingTierFeatures = React.forwardRef<
 ...
 data-d-role="card"className={cn('flex flex-1 flex-col gap-3', className)}
 {...props}
 />
 )
})
...
const PricingTierCta = React.forwardRef<
 ...
 data-d-role="btn"href={target}
 type={target ? undefined : (type ?? 'button')}
 className={cn(
 'mt-auto inline-flex min-h-11 w-full items-center justify-center px-5 py-2.5 text-sm font-medium transition-colors',
 className,
 )}
 onClick={handleClick}
 {...props}
