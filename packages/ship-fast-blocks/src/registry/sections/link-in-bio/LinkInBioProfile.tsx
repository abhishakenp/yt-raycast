...
/**
 * LinkInBioProfile — the full body of a minimal "link in bio" / link-hub page,
 * laid out as a single centered narrow column on a calm, light, paper-toned
 * surface. It opens with a circular avatar headshot ringed in the background
 * color with an "available" status dot, then a name, role/tagline, and short
 * bio. Below stacks a vertical column of large tappable link buttons — each
 * with a leading icon tile, a title + subtitle, and a trailing open-link arrow
 * or a 'New' pill — followed by a centered row of round social icon buttons and
 * a 'Featured Work' card with a wide 16:9 image and caption. Every link, social,
 * and featured item routes through useNavigate; avatar and featured photos use
 * the alt-driven Image component. Use as the main content of a Linktree/Bento
 * style personal landing page, creator/influencer link hub, freelancer bio
 * link, or social-profile splash. Renders fully with no props.
 */
export const LinkInBioProfile = defineComponent({
  name: "LinkInBioProfile",
  description:
    "Full body of a minimal LINK-IN-BIO / link-hub / bio-link page — a single centered narrow column with a calm, light, paper-toned aesthetic. Opens with a circular avatar headshot (ringed in the page color, with an online/available status dot), a name + role/tagline + short bio, then a vertical stack of large tappable link buttons (each with a leading icon tile, title + subtitle, and a trailing open-arrow or 'New' badge), a centered row of round social icon buttons (Twitter/X, GitHub, Dribbble, LinkedIn), and a 'Featured Work' card with a wide 16:9 image and caption. Use as the primary content area of a Linktree / Bento style personal landing page, creator or influencer link hub, freelancer/portfolio bio link, 'all my links in one place' page, or social-profile splash. Supply content only — brand/name, profile, links, socials, featured; the section owns all layout and styling.",
  props: z.object({
