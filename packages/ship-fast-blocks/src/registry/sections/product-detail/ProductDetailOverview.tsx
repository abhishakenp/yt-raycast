...
export const ProductDetailOverview = defineComponent({
  name: "ProductDetailOverview",
  description:
    "Reusable overview / hero section for the Product Detail page family. Derived from the section template catalog to provide section-level coverage without new HTML generation: eyebrow, large heading, supporting copy, dual CTAs, feature pills, KPI strip, and an image panel rendered through the alt-driven Image component. Use when composing a product detail page or adding a focused product detail band to a larger generated site.",
  props: z.object({
  ...
  component: ({ props }) => {
    ...
    const brand = props.brand ?? "Product Detail"
    const eyebrow = props.eyebrow ?? "Product Detail section"
    const heading = props.heading ?? "Product Detail experience, ready to customize"
    const subheading = props.subheading ?? "A reusable overview section derived from the existing Product Detail page family, built for prompts that need a polished first viewport without creating new HTML."
    const primaryCta = props.primaryCta ?? "Explore"
    const secondaryCta = props.secondaryCta ?? "Learn more"
    const imageAlt = props.imageAlt ?? "Product Detail website experience"
    const features = props.features?.length ? props.features : [
      "Purpose-built product detail layout",
      "Token-based styling",
      "Prompt-safe content props"
]
    const stats = props.stats?.length ? props.stats : [
      {
            "value": "01",
    ...
    return (
      <section className={cn("overflow-hidden bg-background py-20 text-foreground sm:py-24", props.className)}>
        <div className="mx-auto grid max-w-7xl gap-12 px-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:px-8">
          <div>
            ...
            <p className="mb-4 text-sm font-semibold uppercase tracking-wider text-primary">{brand}</p>
          ...
          <div className="relative">
            ...
            <div className="relative overflow-hidden rounded-3xl border border-border bg-card shadow-[0_24px_80px_rgba(0,0,0,0.12)]">
              <Image alt={imageAlt} w={900} h={700} className="aspect-[4/3] w-full object-cover" />
              <div className="border-t border-border bg-card/95 p-6">
                ...
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  Section-level building block for generated multi-page experiences.
                </p>
              </div>
            </div>
          </div>
