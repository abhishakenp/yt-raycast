...
export const ProductDetailOverview = defineCapsule({
  name: 'ProductDetailOverview',
  description:
    'Editorial-product overview band for the Product Detail page family. An asymmetric split pairs a content column — mono eyebrow with a primary tick, a small uppercase brand line, an oversized extrabold tight-tracked heading, supporting copy, square mono feature chips, square CTAs (ink-filled primary + hairline outline secondary, both with press feedback), and a collapsed-border KPI ledger with giant tabular values and mono labels — with a sharp hard-offset-shadow image panel rendered through the alt-driven Image component. Use when composing a product detail page or adding a focused product detail band to a larger generated site.',
  props: z.object({
  ...
  component: ({ props }) => {
    ...
    const heading =
      props.heading ?? 'Product Detail experience, ready to customize'
    const subheading =
      props.subheading ??
      'A reusable overview section derived from the existing Product Detail page family, built for prompts that need a polished first viewport without creating new HTML.'
