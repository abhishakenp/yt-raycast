...
export const ProductDetailCta = defineComponent({
  ...
  component: ({ props }) => {
    ...
    const subtitle =
      props.subtitle ?? "Free express shipping and a no-questions 30-day return — premium sound, zero risk."
    const actions = props.actions?.length ? props.actions : [
      { label: "Add to Cart", target: "Overview", variant: "primary" as const },
      { label: "Learn More", target: "Features", variant: "outline" as const },
    ]
