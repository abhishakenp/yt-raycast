    ...
    return (
      <section className={cn("border-t border-border bg-background py-12 sm:py-16", props.className)}>
        <div className="mx-auto max-w-3xl px-6 lg:px-8">
          <h2 className="text-center text-3xl font-bold text-foreground sm:text-4xl">{heading}</h2>
          {subheading ? (
            <p className="mt-3 text-center text-muted-foreground">{subheading}</p>
          ) : null}
            ...
            {items.map((it, i) => (
              ...
              >
                <summary className="flex cursor-pointer list-none items-center justify-between p-5">
                  <h3 className="pr-4 font-medium text-foreground">{it.question}</h3>
