...
export const YogaStudioOverview = defineCapsule({
  name: 'YogaStudioOverview',
  description:
    'Reusable serene, airy overview band for the Yoga Studio page family, restyled on the shared OverviewSection kit slots for section-level coverage without new HTML generation: a soft muted wash with a giant ghost watermark word, an asymmetric split pairing a mono eyebrow, a calm clean-sans heading, grounding copy, mono feature chips, sharp-cornered dual CTAs, and a hairline tabular KPI strip on the left with a hairline double-framed image panel (rendered through the alt-driven Image component) on the right. Use when composing a yoga studio page or adding a focused yoga studio band to a larger generated site.',
  props: z.object({
  ...
  component: ({ props }) => {
    const brand = props.brand ?? 'Yoga Studio'
    const eyebrow = props.eyebrow ?? 'Yoga Studio section'
    const heading =
      props.heading ?? 'Yoga Studio experience, ready to customize'
    const subheading =
      props.subheading ??
      'A reusable overview section derived from the existing Yoga Studio page family, built for prompts that need a polished first viewport without creating new HTML.'
    const primaryCta = props.primaryCta ?? 'Explore'
    const secondaryCta = props.secondaryCta ?? 'Learn more'
    const imageAlt = props.imageAlt ?? 'Yoga Studio website experience'
    const features = props.features?.length
      ? props.features
      : [
          'Purpose-built yoga studio layout',
          'Token-based styling',
    ...
    return (
      ...
      >
        <Watermark className="-top-4 -right-4 text-[7rem] font-semibold tracking-tight sm:text-[11rem] lg:text-[15rem]">
          {brand}
        </Watermark>
        <OverviewGrid className="relative">
          <OverviewContent>
            ...
            <OverviewHeading className="mt-4 text-4xl font-semibold tracking-tight text-foreground sm:text-5xl">
              {heading}
            </OverviewHeading>
            ...
            <OverviewStats className="gap-0 divide-x divide-border border border-border">
              {stats.map((stat: { value: string; label: string }) => (
                <OverviewStat key={stat.label} className="px-5 py-4 first:pl-5">
                  <OverviewStatValue className="text-3xl font-semibold tabular-nums">
                    {stat.value}
                  </OverviewStatValue>
