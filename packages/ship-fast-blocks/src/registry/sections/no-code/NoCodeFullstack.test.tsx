...
vi.mock('@ship-fast/lakebed/react', async () => {
  ...
  return {
    ...
    createLakebedClient: vi.fn(() => {
      if (!lakebedRef.current) throw new Error('Missing no-code Lakebed client')
...
const { NoCodeCta } = await import('./NoCodeCta.tsx')
...
function createNoCodeLakebedStub({
  mutationDelay,
}: {
...
describe('No-code fullstack generated section behavior', () => {
  it('shares plan catalog, command search, Shoo auth, and mobile navigation', async () => {
    const { lakebed, signInWithGoogle, state } = createNoCodeLakebedStub()
    lakebedRef.current = lakebed

    function NoCodeProbe() {
      return (
        <>
          {NoCodeNavbar.client.component({
            props: {
              brand: 'Buildr',
              nav: ['Features', 'Pricing'],
              signInLabel: 'Sign in',
            },
            statementId: 'no_code_navbar',
          })}
          {NoCodePricing.client.component({
            props: {
              plans: [
                {
                ...
                {
                  cta: 'Contact sales',
                  features: ['SSO and advanced security'],
                  featured: true,
                  name: 'Enterprise',
                  period: '',
                  price: 'Custom',
      ...
      expect(state().plans).toEqual([
        {
          ...
          period: '/month',
          price: '$0',
          summary: 'Perfect for side projects',
        },
        {
          name: 'Enterprise',
          period: '',
    ...
    const billingToggle = screen.getByRole('switch', {
      name: 'Toggle yearly billing',
    })
    expect(billingToggle.getAttribute('aria-checked')).toBe('false')
    fireEvent.click(billingToggle)
    expect(billingToggle.getAttribute('aria-checked')).toBe('true')
    expect(navigate).not.toHaveBeenCalledWith('Yearly')
  ...
  it('keeps No-code pricing, hero, and CTA loading scoped to clicked actions', async () => {
    const selectDeferred = createDeferred()
    const demoDeferred = createDeferred()
    const { lakebed, state } = createNoCodeLakebedStub({
      mutationDelay: {
    ...
    function NoCodeProbe() {
      return (
        <>
          {NoCodeHero.client.component({
            props: {
              primaryCta: 'Start 14-day free trial',
              secondaryCta: 'Watch demo',
            },
            statementId: 'no_code_hero',
          })}
          {NoCodePricing.client.component({
            props: {
              plans: [
          ...
          {NoCodeCta.client.component({
            props: {
              primaryCta: 'Start 14-day free trial',
              secondaryCta: 'Schedule a demo',
            },
            statementId: 'no_code_cta',
          })}
        </>
      )
    }
    ...
    const starterButton = screen.getByRole('button', {
      name: 'Start building free for Starter',
    })
