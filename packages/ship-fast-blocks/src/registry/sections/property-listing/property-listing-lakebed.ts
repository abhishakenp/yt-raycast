...
export const propertyListingLakebed = {
  ...
  mutations: {
    recordPropertyInquiry: propertyListing.mutation(
      (_ctx, input: PropertyListingInquiryInput) => {
    ...
    saveListing: propertyListing.mutation(
      (_ctx, input: PropertyListingSaveInput) => {
    ...
    selectListing: propertyListing.mutation(
      (_ctx, input: PropertyListingSelectInput) => {
    ...
    setPropertySearch: propertyListing.mutation(
      (_ctx, input: PropertyListingSearchInput) => {
        const current = _ctx.db.state.orderBy('createdAt').all().at(0)
        const next = {
    ...
    syncPropertyListings: propertyListing.mutation(
      (_ctx, input: { listings: PropertyListingCatalogInput[] }) => {
        const existing = _ctx.db.listings.orderBy('createdAt').all()
        const existingByAddress = new Map(
          existing.map((listing) => [listing.address.toLowerCase(), listing]),
        )

        for (const listing of input.listings as Array<
