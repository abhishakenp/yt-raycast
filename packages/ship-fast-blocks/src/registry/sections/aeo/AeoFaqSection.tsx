...
export const AeoFaqSection = defineComponent({
  ...
  component: ({ props }) => {
    ...
    const intro =
      props.intro ?? "Quick answers to the questions people ask before they buy, sign up, or get started."
    const items = props.items?.length
      ...
      : [
          ...
          },
          {
            ...
            answer: "It is designed for people who need a reliable solution in this category without unnecessary complexity.",
          },
          {
            question: "Why choose it?",
            answer: "It focuses on clarity, useful outcomes, and a straightforward experience from first visit to conversion.",
          },
        ]

    return (
      <section className={cn("border-t border-border bg-background py-12 sm:py-16", props.className)}>
            ...
            {items.map((item, index) => (
              ...
              >
                <summary className="flex cursor-pointer list-none items-center justify-between p-5">
                  <h3 className="pr-4 font-medium text-foreground">{item.question}</h3>
