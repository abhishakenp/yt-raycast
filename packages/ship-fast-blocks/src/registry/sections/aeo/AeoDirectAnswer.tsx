...
export const AeoDirectAnswer = defineComponent({
  ...
  component: ({ props }) => {
    ...
    const whoFor = props.whoFor ?? "Teams and individuals looking for a clear, trustworthy solution in this category."
    const heading = props.heading

    return (
      <section
        className={cn("border-b border-border bg-background py-10 sm:py-12", props.className)}
        aria-label="Overview"
      >
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          {heading ? (
            <h2 className="mb-4 text-2xl font-semibold text-foreground sm:text-3xl">{heading}</h2>
          ) : null}
