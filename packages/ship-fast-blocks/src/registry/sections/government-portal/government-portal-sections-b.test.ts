...
describe('government-portal sections (batch B)', () => {
  ...
  it('Events renders the four-tab notice board', () => {
    ...
    expect(src).toMatch(/label:\s*['"]Tenders['"]/)
    expect(src).toMatch(/label:\s*['"]Notices['"]/)
    expect(src).toMatch(/label:\s*['"]Downloads['"]/)
    expect(src).toMatch(/label:\s*['"]Public Notices['"]/)
    // 4 tab entries in the tabs array
    expect(
      (src.match(/key:\s*['"](tenders|notices|downloads|public)['"]/g) ?? [])
        .length,
    ).toBe(4)
  })
  ...
  it('content sections compose the section-kit SectionHeading', () => {
    for (const { file } of SECTIONS) {
      expect(read(file)).toMatch(importFrom('#/section-kit/SectionHeading.tsx'))
    }
  })
})
