 ...
 */
export const CorporateKimiPage = defineComponent({
  ...
  component: ({ props }) => {
    ...
    const toList = <T,>(value: unknown, fallback: T[]) =>
      Array.isArray(value) && value.length ? (value as T[]) : fallback

    const safeStringArray = (value: unknown, fallback: string[]) => {
      if (!Array.isArray(value)) return fallback
      const out = value
        .map((entry) => (typeof entry === "string" ? safeText(entry) : ""))
        .filter((entry) => entry.length > 0)
      return out.length ? out : fallback
    }

    type CorporateSolutionItem = { title: string; description: string }
    type CorporateStepItem = { title: string; description: string }
    type CorporateOfficeItem = { title: string; caption: string; imageAlt: string }
    type CorporatePricingPlan = {
      name: string
      blurb: string
      price: string
      period: string
    ...
    type CorporateStatItem = { value: string; label: string }
    type CorporateTestimonialItem = {
      quote: string
      name: string
      role: string
      avatarAlt: string
    }
    type CorporateFaqItem = { q: string; a: string }
    type CorporateFooterColumn = { title: string; links: string[] }

    const go = useNavigate()
    const brand = safeText(props.brand, "Nexus")
    const nav = safeStringArray(props.nav, [
      "Solutions",
      "Customers",
      "Pricing",
      "Investors",
    ...
    const heroPrimary = safeText(hero?.primaryCta, "Schedule a Demo")
    const heroSecondary = safeText(hero?.secondaryCta, "Explore Solutions")
    const heroBadges = safeStringArray(hero?.badges, [
      "SOC 2 Type II Certified",
      "ISO 27001 Compliant",
    ])
    const heroImageAlt = safeText(
      hero?.imageAlt,
      "Modern corporate office interior with glass walls and collaborative workspace",
    )
    const heroStatLabel = safeText(hero?.statLabel, "Average ROI")
    const heroStatValue = safeText(hero?.statValue, "340%")

    const logos = toObject<{ heading?: unknown; items?: unknown }>(props.logos)
    const logosHeading = safeText(
      logos?.heading,
      "Trusted by leading enterprises worldwide",
    )
    const logoItems = safeStringArray(logos?.items, [
      "AcmeCorp",
      "Globex",
      "Initech",
      "Hooli",
    ...
    const solutions = toObject<{
      heading?: unknown
      description?: unknown
      items?: unknown
    }>(props.solutions)
    const solutionsHeading = safeText(
      solutions?.heading,
      "Enterprise solutions built for scale",
    )
    const solutionsDesc = safeText(
    ...
    const defaultSolutionItems: CorporateSolutionItem[] = [
      {
    ...
    const solutionItems = toList<unknown>(solutions?.items, defaultSolutionItems).map(
      (entry: unknown, index: number) => {
        const fallback = defaultSolutionItems[index] ?? defaultSolutionItems[0]
        const item = toObject<{ title?: unknown; description?: unknown }>(entry)
        return {
          title: safeText(item?.title, fallback.title),
          description: safeText(item?.description, fallback.description),
        }
      },
    )

    const steps = toObject<{
      heading?: unknown
      description?: unknown
      items?: unknown
    }>(props.steps)
    const stepsHeading = safeText(
      steps?.heading,
      "Implementation in four phases"
    )
    const stepsDesc = safeText(
      steps?.description,
      "Our proven methodology ensures seamless deployment with minimal disruption to your operations."
    )
    const defaultStepItems: CorporateStepItem[] = [
      {
        title: "Discovery",
        description:
    ...
    const stepItems = toList<unknown>(steps?.items, defaultStepItems).map(
      (entry: unknown, index: number) => {
        const fallback = defaultStepItems[index] ?? defaultStepItems[0]
        const step = toObject<{ title?: unknown; description?: unknown }>(entry)
        return {
          title: safeText(step?.title, fallback.title),
          description: safeText(step?.description, fallback.description),
        }
      },
    )

    const offices = toObject<{
      heading?: unknown
      description?: unknown
      items?: unknown
    }>(props.offices)
    const officesHeading = safeText(
      offices?.heading,
      "Global presence, local expertise",
    )
    const officesDesc = safeText(
      offices?.description,
      "14 offices across 6 continents, serving clients in 47 countries with round-the-clock support."
    )
    const defaultOfficeItems: CorporateOfficeItem[] = [
      {
    ...
    const officeItems = toList<unknown>(offices?.items, defaultOfficeItems).map(
      (entry: unknown, index: number) => {
        const fallback = defaultOfficeItems[index] ?? defaultOfficeItems[0]
        const office = toObject<{
          title?: unknown
          caption?: unknown
          imageAlt?: unknown
        }>(entry)
        return {
          title: safeText(office?.title, fallback.title),
          caption: safeText(office?.caption, fallback.caption),
          imageAlt: safeText(office?.imageAlt, fallback.imageAlt),
        }
      },
    )

    const pricing = toObject<{
      heading?: unknown
      description?: unknown
      plans?: unknown
    }>(props.pricing)
    const pricingHeading = safeText(
      pricing?.heading,
      "Transparent enterprise pricing",
    )
    const pricingDesc = safeText(
      pricing?.description,
      "Flexible plans designed to scale with your organization. All plans include implementation support."
    )
    const defaultPricingPlans: CorporatePricingPlan[] = [
      {
        name: "Professional",
    ...
    const pricingPlans = toList<unknown>(pricing?.plans, defaultPricingPlans).map(
      (entry: unknown, index: number) => {
        ...
        return {
          name: safeText(plan?.name, fallback.name),
          blurb: safeText(plan?.blurb, fallback.blurb),
          price: safeText(plan?.price, fallback.price),
          period: safeText(plan?.period, fallback.period),
          features: safeStringArray(plan?.features, fallback.features),
          cta: safeText(plan?.cta, fallback.cta),
          featured:
            typeof plan?.featured === "boolean" ? plan.featured : fallback.featured,
          badge: safeText(plan?.badge, fallback.badge),
        }
      },
    )

    const defaultStatItems: CorporateStatItem[] = [
    ...
    const stats = toObject<{ items?: unknown }>(props.stats)
    const statItems = toList<unknown>(stats?.items, defaultStatItems).map(
      (entry: unknown, index: number) => {
        const fallback = defaultStatItems[index] ?? defaultStatItems[0]
        const item = toObject<{ value?: unknown; label?: unknown }>(entry)
        return {
          value: safeText(item?.value, fallback.value),
          label: safeText(item?.label, fallback.label),
        }
      },
    )

    const testimonials = toObject<{
      heading?: unknown
      description?: unknown
      items?: unknown
    }>(props.testimonials)
    const testimonialsHeading = safeText(
      testimonials?.heading,
      "Trusted by industry leaders",
    )
    const testimonialsDesc = safeText(
      testimonials?.description,
      "See how leading organizations transformed their operations with Nexus."
    )
    const defaultTestimonialItems: CorporateTestimonialItem[] = [
      {
        quote:
    ...
    const testimonialItems = toList<unknown>(
      testimonials?.items,
      defaultTestimonialItems,
    ).map((entry: unknown, index: number) => {
      const fallback = defaultTestimonialItems[index] ?? defaultTestimonialItems[0]
      const testimonial =
        toObject<{ quote?: unknown; name?: unknown; role?: unknown; avatarAlt?: unknown }>(
          entry,
        )
      return {
        quote: safeText(testimonial?.quote, fallback.quote),
        name: safeText(testimonial?.name, fallback.name),
        role: safeText(testimonial?.role, fallback.role),
        avatarAlt: safeText(testimonial?.avatarAlt, fallback.avatarAlt),
      }
    })
    ...
    const faqHeading = safeText(faq?.heading, "Frequently asked questions")
    const faqDesc = safeText(
      faq?.description,
      "Everything you need to know about Nexus enterprise solutions."
    )
    const defaultFaqItems: CorporateFaqItem[] = [
      {
        q: "What is the typical implementation timeline?",
        a: "Most implementations are completed within 90-120 days, depending on complexity and scope. Our phased approach ensures minimal disruption to your operations. Enterprise and Global plans include dedicated project managers to accelerate deployment.",
      },
      {
      ...
      },
      {
        ...
        a: "Yes, our Global plan includes on-premise, hybrid, and private cloud deployment options for organizations with specific data residency or regulatory requirements. Our solutions can be deployed in your own data centers while maintaining the same management and monitoring capabilities.",
      },
      {
      ...
      },
      {
        ...
        a: "Absolutely. Nexus provides comprehensive REST APIs, webhooks, and pre-built connectors for major enterprise systems including Salesforce, SAP, Oracle, Workday, ServiceNow, and 200+ other platforms.",
      },
    ]
    const faqItems = toList<unknown>(faq?.items, defaultFaqItems).map(
      (entry: unknown, index: number) => {
        const fallback = defaultFaqItems[index] ?? defaultFaqItems[0]
        const question = toObject<{ q?: unknown; a?: unknown }>(entry)
        return {
          q: safeText(question?.q, fallback.q),
          a: safeText(question?.a, fallback.a),
        }
      },
    )
    ...
    const ctaHeading = safeText(
      cta?.heading,
      "Ready to transform your enterprise?"
    )
    const ctaDesc = safeText(
      cta?.description,
    ...
    const ctaPrimary = safeText(cta?.primaryCta, "Schedule a Demo")
    const ctaSecondary = safeText(cta?.secondaryCta, "Contact Sales")
    const ctaNote = safeText(
      cta?.note,
      "Average response time: Under 2 hours during business hours",
    )

    const footer = toObject<{
      about?: unknown
      columns?: unknown
      copyright?: unknown
      legal?: unknown
    }>(props.footer)
    const footerAbout = safeText(
      footer?.about,
    ...
    const defaultFooterColumns: CorporateFooterColumn[] = [
      {
        title: "Solutions",
        links: [
    ...
    const footerColumns = toList<unknown>(footer?.columns, defaultFooterColumns).map(
      (entry: unknown, index: number) => {
        const fallback = defaultFooterColumns[index] ?? defaultFooterColumns[0]
        const col = toObject<{ title?: unknown; links?: unknown }>(entry)
        return {
          title: safeText(col?.title, fallback.title),
          links: safeStringArray(col?.links, fallback.links),
        }
      },
    )
    const footerCopyright = safeText(
      footer?.copyright,
      "© 2026 Nexus Enterprise Solutions, Inc. All rights reserved."
    )
    const footerLegal = safeStringArray(footer?.legal, [
      "Privacy Policy",
      "Terms of Service",
      "Cookie Policy",
    ])
