...
function inferCategory(siteType: string, prompt: string): string {
  ...
  if (['software', 'saas', 'dashboard'].includes(type))
    return 'Software product'
  if (/\bblog\b|\bpublication\b|\bnewsletter\b/i.test(prompt))
    return 'Publication'
...
function buildDirectAnswerSection(
  brand: string,
  tagline: string,
  audience: string,
): SectionLike {
  return {
    ...
    body:
      tagline ||
      `${brand} helps customers solve real problems with a clear, trustworthy web presence.`,
  ...
  return {
    ...
    items: [
      ...
      },
      {
        ...
        body:
          audience ||
          `${brand} is built for people who need a reliable solution in this category.`,
      },
      {
...
export function enrichPageAeo(
  ...
  const audience =
    ...
    (/\bfor\b/i.test(prompt)
      ? promptSnippet(prompt, 120, 'its target audience')
      : 'its target audience')

  const aeo: PageAeoContract = {
    objective:
      page.aeo?.objective ||
      `Explain what ${brand} does and who it serves on ${page.route || '/'}`,
  ...
  const hasDirectAnswer = sections.some(
  ...
  if (page.route === '/' && !hasDirectAnswer) {
    ...
    sections.splice(
      insertAt,
      0,
      buildDirectAnswerSection(
        brand,
        String(page.description || siteSpec.seo?.description || ''),
        audience,
      ),
    )
  }
...
export function enrichSiteSpecAeo(
  siteSpec: SiteSpecLike,
  prompt = '',
): SiteSpecLike {
  const brand = String(
    siteSpec.projectName ||
      siteSpec.seo?.siteName ||
      promptSnippet(prompt, 40, 'Generated Project'),
  )
  const siteType = String(siteSpec.siteType || 'landing')
  const category = inferCategory(siteType, prompt)
  const description = String(
    siteSpec.seo?.description || `${brand} — ${category}`,
  )
  ...
  const seo = {
    ...
    keywords: siteSpec.seo?.keywords || [
      brand.toLowerCase(),
  ...
  const pages = (siteSpec.pages || []).map((page) =>
    enrichPageAeo(page, { ...siteSpec, projectName: brand, seo }, prompt),
  )
