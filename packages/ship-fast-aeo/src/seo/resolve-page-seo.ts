...
export function resolvePageSeo(siteSpec: SiteSpecLike | null | undefined, page: SitePageLike | null | undefined): ResolvedPageSeo {
  ...
  const routePath = normalizePath(String(pageSeo.canonicalPath || page?.route || '/'))
  ...
  const description = String(pageSeo.description || page?.description || siteSeo.description || '').trim()
  ...
  const ogImage = resolveAssetUrl(String(pageSeo.ogImage || siteSeo.ogImage || ''), siteUrl)
  const ogImageAlt = String(pageSeo.ogImageAlt || siteSeo.ogImageAlt || `${title || siteName} social preview`).trim()
