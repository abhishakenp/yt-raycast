...
def parse_and_write_files(output: str, workspace: str, task: dict) -> list:
    ...
    # Strategy 4: raw HTML content (pre-generated by ship.py) — write directly to first task file
    if not written and task_files:
        stripped = output.strip()
        if stripped.startswith("<!DOCTYPE") or stripped.startswith("<html"):
            resolved = os.path.join(workspace, task_files[0])
            _write_file(resolved, stripped, workspace, written)
...
def run_tasks():
    ...
    def generate_for_task(task):
        ...
        # Detect pre-generated HTML content in action (from ship.py parallel generation)
        # If action already contains a complete HTML file, write it directly — no need to call Groq again
        action_stripped = action.strip()
        if (action_stripped.startswith("<!DOCTYPE") or action_stripped.startswith("<html")) and len(action_stripped) > 500:
            log(f"  {tid}: pre-generated HTML detected ({len(action_stripped)} chars) — writing directly")
            return {"content": action_stripped, "is_command": False, "pre_generated": True}
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def _handle_reload(self):
        """Reload tasks from tasks.json (used after reset when executor is reused)."""
        global is_running
        workspace = os.environ.get("IRIS_WORKSPACE", os.getcwd())
        ...
        try:
            ...
            is_running = False
            log(f"[reload] Reloaded {len(tasks_data)} tasks")
            emit("tasks_loaded", {"count": len(tasks_data)})
        except (FileNotFoundError, json.JSONDecodeError) as e:
            tasks_data.clear()
            log(f"[reload] Cleared tasks (file not ready: {e})")

        self.send_response(200)
        self.send_header("Content-Type", "application/json")
        self._cors()
        self.end_headers()
        self.wfile.write(json.dumps({"ok": True, "count": len(tasks_data)}).encode())
...
def load_tasks():
    ...
    try:
        ...
        # Normalize statuses: LLMs may output "TODO" but executor expects "PENDING"
        for t in tasks_data:
            status = t.get("status", "").upper()
            if status not in ("DONE", "FAILED", "PENDING", "IN_PROGRESS"):
                t["status"] = "PENDING"
...
def main():
    ...
    if not os.environ.get("SHIP_NO_BROWSER"):
