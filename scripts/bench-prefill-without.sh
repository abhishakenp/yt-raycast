...
echo "=== WITHOUT prefill (normal low thinking) ==="
for i in 0 1 2 3 4; do
  echo "--- Prompt $((i+1)) ---"
  DISABLE_HARMONY_PREFILL=1 HOMEPAGE_MODEL=openai/gpt-oss-120b timeout 60 bun scripts/bench-quality.mjs "${PROMPTS[$i]}" "$WITHOUT_DIR/prompt-$((i+1)).txt" 2>&1
done
