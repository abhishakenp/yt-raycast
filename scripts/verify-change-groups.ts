...
export const changeGroups: ChangeGroup[] = [
  {
    ...
    patterns: [
      ...
      /^convex\/(?:billing|brandfetch|customLanguages|export_artifacts|github|lakebed|lakebed_deploy|openui_ssr_health|translationCache)\.ts$/,
      /^convex\/lib\/(?:chat_refinement|export_progress_stages|session_|inline_edit_br_persistence_contract|cms_helpers|module_naming|openui_error_html).*\.(?:test\.)?ts$/,
  ...
  },
  {
    ...
    patterns: [
      ...
      /^dogfood-output\//,
      /^public\/scripts\//,
      /^scripts\/verify-(?:browser-helpers|generation-agent-browser|chat-browser|chat-refinement|cms-browser)\.mjs$/,
      /^src\/app\/providers\//,
      /^src\/components\//,
      /^src\/features\/(?:admin|agentation|brand|chat|cms|clone|dashboard|editing|exports\/server|gallery|generation\/server|generation\/services|home|localization|preview|referrals|session)\//,
      ...
      /^src\/genui\/components\/(?:LanguagePicker\.behavioral\.test|LanguagePicker|ThemePicker(?:\.test)?|localization-rail\.test)\.tsx?$/,
      /^src\/genui\/(?:theme-apply|theme-runtime)\.(?:test\.)?ts$/,
      /^src\/hooks\//,
      /^src\/lib\/home\//,
      /^src\/lib\/(?:chunk-load-recovery|content-policy|escape-html|glass-pill-html|image-query|rate-limit|safe-fetch|site-config|structured-data|utils)\.(?:test\.)?ts$/,
