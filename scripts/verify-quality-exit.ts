...
type QualityExitOptions = {
  requireChangeReport?: boolean
  requireReviewBundles?: boolean
}
...
export function verifyQualityExitFromTexts(
  ...
  {
    requireChangeReport = true,
    requireReviewBundles = true,
  }: QualityExitOptions = {},
) {
...
export function verifyQualityExit() {
  const changeScope = readChangeScope()
  const changedPaths = changeScope.entries.map((entry) => entry.path)
  ...
  if (changedPaths.length === 0) {
    return verifyQualityExitFromTexts(
      ...
      { requireChangeReport: false, requireReviewBundles: false },
    )
  }
