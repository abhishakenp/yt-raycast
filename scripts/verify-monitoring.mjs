...
import { buildConvexCliEnv } from './verify-provider-gate-lib.mjs'
...
const convexCliEnv = buildConvexCliEnv(process.env)
...
convexRun('internal.sessions.completeGeneration', {
  ...
  siteSpecJson: JSON.stringify({
    ...
    pages: [
      { id: 'home', title: prompt, description: 'Monitoring verifier page' },
    ],
  }),
...
assert(
  metrics.totalElapsedMs >= 4321,
  'usage metrics did not record elapsed time',
)
assert(
  metrics.byProvider['ship-fast-engine'] >= 1,
  'usage metrics missing ship-fast-engine provider',
)
assert(
  metrics.byEventType.run_completed >= 1,
  'usage metrics missing run_completed event',
)

const stream = convexRun('sessions:getEventStream', { lookup: sessionId })
const events = Array.isArray(stream.events) ? stream.events : []
assert(
  events.some((event) => event.eventType === 'preview_ready'),
  'event stream missing preview_ready',
)
...
assert(
  completed.provider === 'ship-fast-engine',
  'run_completed provider mismatch',
)

const sessionApi = convexRun('sessions:getSessionApiResponse', {
  lookup: sessionId,
})
assert(
  sessionApi.status === 'preview_ready',
  'session API did not report preview_ready',
)
