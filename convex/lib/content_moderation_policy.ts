...
const regexRules: Array<[ModerationCategory, string, RegExp]> = [
  ...
  [
    'hate_extremism',
    'hate-inferiority-superiority',
    /\b(race|racial|ethnic|religious)\w*\s+(is|are)\s+(inferior|superior|subhuman|less\s+than|lesser)\b/i,
  ],
  [
    'hate_extremism',
    'hate-expel-deport',
    /\b(expel|deport|eradicate|exterminate|remove)\w*\s+(all|every)\w*\s+(race|ethnic|religious|minority|group|people)\w*\b/i,
  ],
  [
    'hate_extremism',
    'hate-protected-group-target',
    /\b(protected\s+ethnic|racial|religious)\s+(group|minority)\w*\s+(is|are)\s+(inferior|subhuman|evil|dangerous|threat)\b/i,
  ],
]
