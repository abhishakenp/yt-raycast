...
function stableArtifactInput(
  ...
  const finalHtml = prepared.previewHtml?.trim()
  ...
  return {
    artifact: { html: finalHtml, ...(siteSpec === undefined ? {} : { siteSpec }) },
    sessionId: prepared.sessionId,
    target: prepared.target,
    theme: {
    ...
    ...(options.formatCache === undefined ? {} : { formatCache: options.formatCache }),
