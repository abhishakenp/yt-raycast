...
describe('export_artifacts build action', () => {
  ...
  it('builds a final HTML artifact through the stable export boundary', async () => {
    ...
    ;(
      buildExportFromStableArtifact as ReturnType<typeof vi.fn>
    ).mockResolvedValue({
      files: { 'index.html': download.body as string },
      download,
    })
    ;(
      buildDownloadFromStableArtifact as ReturnType<typeof vi.fn>
    ).mockResolvedValue(download)
    ...
    expect(buildExportFromStableArtifact).toHaveBeenCalledWith(
      expect.objectContaining({
        artifact: expect.objectContaining({
          html: expect.stringContaining('<h1>Stable</h1>'),
        }),
        target: 'html',
      }),
    )
