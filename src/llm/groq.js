...
- Ecommerce (Luxury DTC, editorial): build a FULL retail homepage (match structural depth of exemplar URLs — ${ECOMMERCE_REFERENCE_EXEMPLAR_URLS.join(' | ')}), with premium restraint and editorial craft: promo/benefit strip, sticky dense header (search input, account, cart badge, multi-level shop nav/collections), art-directed hero with product/lifestyle imagery (or elegant non-photo treatment), benefit chips, four-plus collection/category tiles, six-plus premium product cards, bundle/gift set band, editorial story/learn section, testimonials/reviews/press, newsletter, fat multi-column footer with policy links. Do not copy third-party brands or assets. BANNED: sparse SaaS-style storefronts with only hero + 3 cards + footer. FORBIDDEN as the dominant above-the-fold story for storefronts: pricing comparison tables, three-column icon "features", symmetric bento-as-hero, dashboard/login framing, or marketing-page pill badge + single CTA without product or category merchandising. REQUIRED: visible shop cues early—product or lifestyle imagery, category entry points, prices + add-to-cart paths, cart/search in header.\n+  Editorial constraints:\n+  - Desktop collections module MUST be a 2x2 (or 4-up) grid with equal tile widths. Only mobile may use horizontal scrolling; do not use pager dots on desktop.\n+  - Curated sets MUST be a complete 2-up editorial module (two equal cards), each with image (or intentional graphic panel), title, 1-line descriptor, anchored price, and a labeled CTA.\n+  - Materials section MUST be a deliberate editorial grid (no accidental whitespace). Use either: (A) image with caption + 3 bullets; or (B) text + pull quote + inset image.\n+  - Reviews MUST include credibility cues: reviewer name, product name, and a subtle “Verified purchaser” and date. Add a rating summary line (e.g. “Rated 4.8/5 from 1,200 reviews”).\n+  - Newsletter MUST have a visible submit button label (e.g. “Subscribe”) and a one-line privacy note (“No spam. Unsubscribe anytime.”). Empty/blank buttons are forbidden.\n+  - Social icons: do NOT rely on lucide icon names that might be missing. Prefer plain text links (Instagram, X, Facebook) or inline SVGs.\n+  Product cards MUST include: image, label/badge, title, price, rating/review count (de-emphasized), and a single trust microline (one clause only) plus a clear CTA. Copy tone: quiet luxury, specific materials/craft, no generic filler. Also study ${ECOMMERCE_AWWWARDS_GALLERY_URL}, ${ECOMMERCE_ENVATO_TEMPLATES_URL}, and ${ECOMMERCE_DRIBBBLE_TAG_URL} (patterns and craft only — no copying specific shots). Medusa (${ECOMMERCE_MEDUSA_DOCS_LEARN}): Store API, cart, products, regions — editorial type, strong product imagery, many distinct sections. Any section titled like a carousel, "curated/gift sets", bundles, or "shop the …" row MUST place **at least three** product cards in the same horizontal strip (one shared \`grid\` or \`flex flex-row gap-*\` containing multiple cards)—never a single product tile with a large empty area beside it.
