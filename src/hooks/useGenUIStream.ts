...
function setSection(
  ...
  if (idx === -1)
    return [...sections, { id, status: 'pending', attempt: 0, ...patch }]
...
function reduce(prev: GenUIState, ev: GenUIEvent): GenUIState {
  switch (ev.type) {
    ...
    case 'module_start':
      return {
        ...prev,
        sections: setSection(prev.sections, ev.id, { status: 'building' }),
      }
    case 'module_retry':
      return {
        ...prev,
...
export function useGenUIStream() {
  ...
  const start = useCallback(async (prompt: unknown, model?: unknown) => {
    ctrl.current?.abort()
