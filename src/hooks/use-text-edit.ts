...
/**
 * Hook that enables inline text editing in a DOM container.
 *
 * When `editMode` is true:
 *   1. Clicking any text element makes it contentEditable
 *   2. On blur/Enter the edited text is captured
 *   3. `onTextChange` is called with { oldText, newText }
 *
 * The caller is responsible for updating the program string and re-rendering.
 */
...
function findTextElement(el: HTMLElement): HTMLElement | null {
	...
	while (current) {
		...
		if (
			["button", "a", "input", "textarea", "select", "svg", "path", "img", "video", "audio", "script", "style"].includes(tag)
		) {
			return null;
		}
		...
		if (text.length > 0 && text.length < 500) {
			if (
				["h1", "h2", "h3", "h4", "h5", "h6", "p", "span", "div", "li", "td", "th", "label", "figcaption", "strong", "em", "small", "blockquote"].includes(tag)
			) {
				return current;
			}
		}
