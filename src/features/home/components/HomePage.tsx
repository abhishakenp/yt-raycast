...
const EXAMPLE_CHIPS = [
  ['Image studio', 'This app is going to be an image generation studio using various AI models to turn a prompt into images. Design a mocked version (no backend). It should be dark mode. Focus on making it beautiful.'],
  ['Pet wellness', 'Build a bold landing page for a premium pet wellness app with a booking section and customer testimonials.'],
  ['SaaS dashboard', 'Create a clean SaaS marketing dashboard for a remote team productivity platform with charts and responsive cards.'],
  ['Hindi gym site', 'Mere local gym ke liye ek powerful modern website banao with membership plans'],
]

const pillButton = (id: string, label: string, className = '') =>
  glassPillButtonHtml({
    type: 'button',
    id,
    className,
...
const renderExamplePromptChips = () =>
  EXAMPLE_CHIPS.map(
    ([label, text], index) =>
      `<button type="button" class="dev-prompt-chip" data-prompt="${text}" title="${text}">
        <span class="dev-prompt-chip-num">${index + 1}</span>
        <span class="dev-prompt-chip-label">${label}</span>
      </button>`,
  ).join('')
...
<nav class="top-actions" aria-label="Primary">
  <div class="top-actions-right">
    ...
    <div class="top-actions-auth-slot">
      ${pillButton('signin-btn', 'Sign in', 'pill--top-actions')}
      ${glassPillButtonHtml({
        className: 'pill--top-actions',
        id: 'signout-btn',
        html: '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" x2="9" y1="12" y2="12"/></svg> Sign out',
      })}
    </div>
  </div>
</nav>
...
<div id="auth-overlay" class="hidden" role="dialog" aria-modal="true" aria-labelledby="auth-title" aria-describedby="auth-description" aria-hidden="true">
  <div class="auth-box" role="document">
    ...
    <form id="auth-email-form" class="auth-email-form" novalidate>
      ...
      <div class="auth-email-row">
        ${pillButton('email-signin-btn', 'Sign in', '')}
        ${pillButton('email-signup-btn', 'Create account', '')}
      </div>
    </form>
...
<div class="private-gen-modal" id="private-gen-modal" aria-hidden="true">
  ...
  <div class="private-gen-modal-card" role="dialog" aria-modal="true" aria-labelledby="private-gen-modal-title">
    <button class="pill pill--modal-close" id="private-gen-modal-close" aria-label="Close">
      <span class="pill__body">Close</span>
    </button>
...
<div class="page-layout">
  <div class="container sidebar-panel">
    <h1 class="sr-only">Ship Fast — AI website generator</h1>
    ...
    <section class="launch-hero" aria-label="Print your mind in seconds">
      ...
      <div class="launch-stage">
        <div class="launch-prompt-stack">
            ...
            <div class="hero-card-inner">
              <form id="prompt-form" class="input-group">
                ...
                <div class="prompt-field">
                  ...
                  <div class="prompt-suggestions" id="prompt-suggestions" hidden>
                    <ul class="prompt-suggestions-list" id="prompt-suggestions-list" role="listbox" aria-label="Prompt ideas"></ul>
                  </div>
                ...
                <div class="design-ref-panel" id="design-ref-panel">
                  <div class="design-ref-search-wrap">
                    <svg class="design-ref-search-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
                    <input
                      class="design-ref-search"
                      type="text"
                  ...
                  <div class="design-ref-preview" id="design-ref-preview">
                    <img class="design-ref-preview-favicon" id="design-ref-preview-favicon" alt="" />
                    <div class="design-ref-preview-info">
                      <div class="design-ref-preview-title" id="design-ref-preview-title"></div>
                      <div class="design-ref-preview-url" id="design-ref-preview-url"></div>
                    </div>
                    ${glassPillButtonHtml({
                ...
                <div class="prompt-form-footer">
                  ...
                  ${glassPillButtonHtml({
                    ...
                    id: 'submit-btn',
                    disabled: true,
                    html: '<svg class="zap-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" /></svg><span class="btn-label">Generate</span><div class="spinner"></div>',
                  })}
                </div>
              </form>

              <div class="prompt-policy-violation" id="prompt-policy-block" role="alert" aria-live="assertive" hidden></div>
              ...
              <div class="share-bonus-panel" id="share-bonus-panel" style="display:none">
                ...
                <div class="share-bonus-icons">
                  ...
                  ${pillButton(
                    'bonus-share-native',
                    '<svg class="share-icon-svg share-icon-svg-stroke" viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" x2="12" y1="2" y2="15"/></svg>',
                    'share-icon share-icon-native pill--share-32',
                  )}
                </div>
          ...
          <div class="launch-right-panel">
            <div class="dev-prompt-chips dev-prompt-chips--glass" aria-label="Example prompts">
...
export const HomePage = () => {
  useEffect(() => {
    ...
    void import('@/scripts/launch-backdrop').catch(() => {
      // Non-critical interactive effect.
    })
    void import('@/scripts/homepage').catch(() => {
      // Interactive logic is critical for form controls; keep silent but visible in devtools.
    })
  ...
  return <div className="sf-homepage-root" dangerouslySetInnerHTML={{ __html: homePageHtml }} />
}
