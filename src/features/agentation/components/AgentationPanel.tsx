...
export const AgentationPanel = ({ sessionId }: AgentationPanelProps) => {
  const {
    ...
    remove,
    sendAnnotations,
  } = useAgentationController(sessionId)
  ...
  return (
    <div className="rounded-[var(--radius-xl)] bg-[var(--glass-bg)] p-4 shadow-[var(--glass-shadow)] border border-[var(--glass-border)] backdrop-blur-[12px]">
      ...
      <div className="mb-3 flex max-h-48 flex-col gap-2 overflow-y-auto [scrollbar-color:var(--border-primary)_transparent] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-[3px] [&::-webkit-scrollbar-thumb]:bg-[var(--border-primary)]">
        {annotations?.map((ann) => (
          <div
            className="rounded-[var(--radius-sm)] border border-[var(--border-primary)] bg-[var(--bg-input)] p-3 transition-all duration-200 ease-[var(--ease-out)] hover:border-[var(--border-hover)] hover:bg-white/5"
            key={ann.annotationId}
          >
            <div className="flex items-start justify-between gap-2">
              <div className="flex-1">
                <p className="m-0 mb-1 text-[0.7rem] font-bold uppercase tracking-[0.1em] text-[var(--text-muted)]">
                  {ann.elementLabel}
                </p>
                <p className="m-0 text-sm leading-[1.4] text-[var(--text-secondary)]">
                  {ann.comment}
                </p>
              </div>
              <button
        ...
        {annotations?.length === 0 && (
          <p className="py-8 text-center text-sm italic text-[var(--text-muted)]">
            No annotations yet. Use AI agent to add annotations.
          </p>
        )}
      </div>
      <button
