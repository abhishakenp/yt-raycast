...
home = Stack([home_hero_anchor, home_pricing_anchor])`
    ...
    expect(String(warn.mock.calls[0]?.[0])).toContain('[ship-fast] Deprecated:')
    expect(String(warn.mock.calls[0]?.[0])).toContain('buildSectionMoveCommand')
  })
  ...
  it('warns when the legacy replacementOpenUiSource section rewrite path is used', () => {
    ...
    expect(warn).toHaveBeenCalledTimes(1)
    expect(String(warn.mock.calls[0]?.[0])).toContain('[ship-fast] Deprecated:')
    expect(String(warn.mock.calls[0]?.[0])).toContain('replacementOpenUiSource')
  })
