...
describe('getElementStyleAnchor', () => {
  ...
  it('escapes ids that are not valid bare CSS identifiers', () => {
    ...
    expect(root.querySelector(anchor!)).toBe(
      queryHTMLElement(root, 'section'),
    )
  })
...
describe('legacy capsule marker deprecation warnings', () => {
  ...
  it('warns separately for distinct capsules', () => {
    ...
    expect(
      messages.some((message) => message.includes('ProbeOne')),
    ).toBe(true)
    expect(
      messages.some((message) => message.includes('ProbeTwo')),
    ).toBe(true)
  })
