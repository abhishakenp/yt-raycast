...
interface InlineEditToolbarProps {
  ...
  onStyleApply: (outerHTML: string) => void
  onClose: () => void
  isApplying?: boolean
}
...
export function InlineEditToolbar({
  ...
  useEffect(() => {
    ...
    setColor(computed.color || '#ffffff')
  ...
  }, [isOpen, onClose, activeElement, isApplying])
  ...
  const handleApply = () => {
    if (activeElement && !isApplying) {
      onStyleApply(activeElement.outerHTML)
    }
  }
