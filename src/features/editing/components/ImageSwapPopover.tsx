    ...
    const searchImages = async () => {
      ...
      try {
        const queries = [
          ...
          `${searchQuery} high quality`,
        ]
        const searchResults: SearchResult[] = []

        for (const query of queries.slice(0, 6)) {
          const result = await resolveStockImage({
            alt: query,
            w: 400,
            h: 300,
            context,
                ...
                {results.map((result, index) => (
                  ...
                  >
                    ...
                    <div className="absolute bottom-1.5 left-1.5 right-1.5">
                      <span className="inline-block max-w-full truncate rounded bg-black/60 px-1.5 py-0.5 text-[10px] text-white/90">
                        {result.source}
                      </span>
                    </div>
                  </button>
