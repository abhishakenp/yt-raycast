...
describe('inspect tvnl lakebed build', () => {
  ...
  // Skipped via skipIf when TVNL_SESSION is absent (loaded from .env by
  // vitest.setup.ts). The Convex deployment has VITE_DISABLE_CLERK=true so
  // anonymousOwnerSecret is not required — any session is accessible.
