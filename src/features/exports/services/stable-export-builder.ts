...
export async function buildExportFromStableArtifact(
  ...
  if (target === 'html') {
    ...
    const files = createHtmlExportFiles(sessionId, 'html', download.body, {
...
function buildStaticFrameworkFiles(
  input: StableExportInput,
...
export async function buildDownloadFromStableArtifact(
  ...
  if (input.target === 'html') {
    ...
    const safePreviewHtml = !isUnsafePublicPreviewHtml(artifactHtml)
      ? artifactHtml
      : undefined
  ...
  const slugSource =
    input.target === 'lakebed' ? input.sessionId : metadata.title
  const slug =
    slugSource
      .trim()
      .toLowerCase()
