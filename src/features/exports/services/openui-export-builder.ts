...
function renderTranslatedQuery(
  ...
  const specializedDefaults: Record<string, string> = {
    cartSummary: '{ count: 0, items: [] }',
    commerceSearchState: "{ query: '', selectedLabel: '' }",
    productCatalog: '[]',
    subscriberSummary: '{ count: 0 }',
  }
  const defaultValue = specializedDefaults[queryName]
    ? `(${specializedDefaults[queryName]})`
    : fallback
      ? `(${fallback.trim()})`
      : `[]`
...
function translateBlockSourceLakebed(
  ...
  body = body.replace(/\bauth=\{lakebed\}/g, 'auth={siteAuth}')
  ...
  const needsSiteAuth = /\bsiteAuth\b/.test(body)
  if (needsSiteAuth) dataKeys.usesAuth = true
  ...
  if (needsSiteAuth) {
    extraImports.push(`import { siteAuth } from '${relPrefix}lib/auth'`)
  }
...
function renderPortableCommerceConfig(
  siteSpecJson: string | undefined,
): string {
  const commerce = parseSiteSpec(siteSpecJson).commerce
  if (!isObj(commerce)) return ''
  const backendUrl =
    typeof commerce.backendUrl === 'string' ? commerce.backendUrl.trim() : ''
  const storefrontUrl =
    typeof commerce.storefrontUrl === 'string'
      ? commerce.storefrontUrl.trim()
      : ''
  if (!backendUrl && !storefrontUrl) return ''
  return `

export const commerceBackendUrl = ${JSON.stringify(backendUrl)}
export const commerceStorefrontUrl = ${JSON.stringify(storefrontUrl)}
`
}
...
function renderReactApp(routes: ExportRoute[]): string {
  ...
  const routeElements = routes
    .map(
      (route) =>
        `<Route path={${route.componentName}Route.path} element={<${route.componentName} />} />`,
    )
    .join('\n        ')
...
function stripGeneratedOwnedTypes(files: Record<string, string>): void {
  function transformer(
    context: ts.TransformationContext,
  ): ts.Transformer<ts.SourceFile> {
    function stripParameterType(
  ...
  for (const [path, source] of Object.entries(files)) {
    if (!/^src\/.*\.tsx?$/.test(path) || path.includes('/vendor/')) continue
    const sourceFile = ts.createSourceFile(
      path,
      source,
      ts.ScriptTarget.Latest,
    ...
    const transformedFile = result.transformed.find(ts.isSourceFile)
    if (transformedFile) files[path] = printer.printFile(transformedFile)
    result.dispose()
  }
}
