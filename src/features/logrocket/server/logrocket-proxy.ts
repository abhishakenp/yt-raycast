 ...
 */
export function buildUpstreamUrl(target: ProxyTarget, upstreamPath: string): string {
  const base = UPSTREAMS[target]
  const path = upstreamPath.includes('?')
    ? upstreamPath
    : upstreamPath
  return base + path
}
 ...
 */
export async function proxyLogRocketRequest(request: Request): Promise<Response> {
