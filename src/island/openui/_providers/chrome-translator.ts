...
function getFactory(): ChromeTranslatorFactory | null {
  ...
  return f && typeof f.availability === "function" && typeof f.create === "function" ? f : null;
}
...
async function getTranslator(target: string): Promise<ChromeTranslator | null> {
  ...
  const created = (async () => {
    try {
      ...
      if (!avail) {
        avail = factory.availability({ sourceLanguage: SOURCE, targetLanguage: target });
...
export async function translateOnDevice(text: string, locale: string): Promise<string | null> {
