...
const ensureSfGlassSvgFilters = (): void => {
...
class SfGlassPill extends HTMLElement {
  connectedCallback(): void {
    ...
    btn.type = (this.getAttribute('type') as HTMLButtonElement['type']) || 'button'

    const extra = this.getAttribute('data-pill-class') || ''
    btn.className = extra ? `pill ${extra}` : 'pill'

    ATTRS.forEach((attrName: AttrName) => {
    ...
    const textNode = btn.querySelector<HTMLElement>('.pill__txt')
