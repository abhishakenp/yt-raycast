...
export const isNextPreviewFeatureEnabled = () => {
  ...
  if (process.env.NODE_ENV === 'production' && process.env.SHIP_FAST_NEXT_PREVIEW !== '1') return false
  return true
}
...
const writeNextEnvLocal = (workspace) => {
...
const freePort = (port) => {
  try {
    execFileSync('sh', ['-c', `kill $(lsof -t -i:${port}) 2>/dev/null || true`], { stdio: 'ignore' })
  } catch {
    void 0
  }
}
...
const runBunInstall = (cwd) =>
  new Promise((resolve, reject) => {
    ...
    p.on('error', reject)
    p.on('exit', (code) => (code === 0 ? resolve() : reject(new Error('bun install failed in next-app'))))
  })
...
export const getNextPreviewSnapshot = (session) => {
  ...
  const running =
    previewState.sessionId === session.id && previewState.child && previewState.child.exitCode === null
  return {
    enabled: true,
    ecommerce,
...
export const startNextPreview = async (session, broadcast) => {
  ...
  if (previewState.sessionId === session.id && previewState.child && previewState.child.exitCode === null) {
    const url = `http://127.0.0.1:${previewState.port}`
    return { ok: true, running: true, url }
  }
  freePort(previewState.port)
  writeNextEnvLocal(session.workspace)
...
export const stopNextPreview = (sessionId) => {
  if (!previewState.child || previewState.sessionId !== sessionId) return { ok: true, stopped: false }
  try {
    previewState.child.kill('SIGTERM')
  } catch {
    void 0
