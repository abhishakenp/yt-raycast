...
function brandCaseWall(label, a) {
  return `<div class="absolute inset-4 grid grid-cols-3 gap-3">
    ${Array.from({ length: 9 }, (_, index) => `<div class="relative overflow-hidden rounded-lg border border-white/15 bg-[${index % 3 === 0 ? a.bg : index % 3 === 1 ? a.surface : a.accent}]/75 p-3">
...
export function renderArtDirectedImageSurface(subject, className, plan, index, route) {
  ...
  return shell(label, className, plan, index, route, kind, render(label, a))
}
