...
const tenantToken = (sessionConfig?.token || sessionConfig?.writeToken || '').trim()
const envToken = (
...
const studioToken = tenantToken || envToken
...
const authStore =
  projectId && dataset && studioToken
    ? createMockAuthStore({
        ...
        currentUser: {
          id: 'api-token',
          name: sessionConfig?.sessionId
            ? `Tenant ${sessionConfig.sessionId.slice(0, 8)}`
            : 'API token',
          email: '',
          role: 'editor',
