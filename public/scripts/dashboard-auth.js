...
var GITHUB_TOKEN_STORAGE_KEY = "sf_github_access_token";
var auth = null;
var currentUser = null;
var ready = initDashboardAuth();
...
async function ensureGithubRepoAccessToken() {
  ...
  try {
    result = hasGithubProvider ? await reauthenticateWithPopup(user, provider) : await linkWithPopup(user, provider);
  } catch (error) {
    if (!hasGithubProvider && isFirebaseAuthError(error) && error.code === "auth/provider-already-linked") {
